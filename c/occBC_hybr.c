...
int sol_zproc(struct st_wr *st, gsl_vector * ss, gsl_matrix * xss, struct shock_mats * mats0){
	...
	for(estiter=0;estiter<maxestiter;estiter++){
		...
		for(di=0;di<simT;di++){
			...
			double Z_fd = 0.;
			for(d=0;d<Noccs;d++) Z_fd +=   zz_invth[d]*xd[d];// /(double)Noccs
			if(Z_fd<zlb){
				Z_fd = zlb;}
		...
		*/
		...
		for(d=0;d<Noccs;d++) zzmean_esti[d] = 0.;
		for(di=0;di<simT;di++){
			Zmean_esti += exp(gsl_matrix_get(Zz_hist,di,0))/(double)simT;
			for(d=0;d<Noccs;d++)
				zzmean_esti[d] += exp(gsl_matrix_get(Zz_hist,di,d+Notz))/(double)simT;
		}
		Zmean_esti = log(Zmean_esti);
		for(d=0;d<Noccs;d++) zzmean_esti[d] = log(zzmean_esti[d]);
		for(di=0;di<simT;di++){
			double Zhere = gsl_matrix_get(Zz_hist,di,0) - Zmean_esti;
			gsl_matrix_set(Zz_hist,di,0,Zhere);
			for(d=0;d<Noccs;d++){
				double zzhere = gsl_matrix_get(Zz_hist,di,d+Notz) - zzmean_esti[d];
				gsl_matrix_set(Zz_hist,di,d+Notz,zzhere);
			}
...
int sim_moments(struct st_wr * st, gsl_vector * ss,gsl_matrix * xss){
	...
	if(printlev>=2){
		s_mom.Fdur = gsl_vector_calloc(6);
		s_mom.Fdur_occ = gsl_vector_calloc(6);
		s_mom.Ft = gsl_vector_calloc(Ndraw);
		s_mom.xFdur = gsl_vector_calloc(6);
...
int dur_dist(struct dur_moments * s_mom, const gsl_matrix * gld_hist, const gsl_matrix * pld_hist, const gsl_matrix * x_u_hist, const gsl_matrix * x_ust_hist) {
	...
	int Ndir  = Noccs*(Noccs+1);

	double durs[] = {0., 1.0, 3.0, 6.0, 12.0, 24.0};
	...
	for (duri = 0; duri < 6; duri++) { // loop over each value of vector durs
	...
	for(t=0;t<Ndraw;t++){
		for (duri = 0; duri < 24; duri++) {
			for(d=0;d<Noccs;d++){
				for (l = 0; l < Noccs + 1; l++) durpop_t_ldur[t][duri*Ndir + l*Noccs + d] = 0.;
			}
		}
		for (l = 0; l < Noccs + 1; l++) {
			Ft_l[t][l] = 0.;
	...
	for(t=1;t<Ndraw;t++){
		for(l=0;l<Noccs+1;l++){
			Ft_l[t][l] = 0.;
			for(d=0;d<Noccs;d++)
				Ft_l[t][l] += gsl_matrix_get(gld_hist, t-1, l*Noccs + d)*gsl_matrix_get(pld_hist, t-1, l*Noccs + d);
			for(d=0;d<Noccs;d++){
	...
	for(t=0;t<Ndraw;t++){
		double dursd_t =0.,dur_t_l,pop_t_l;
		for(l=0;l<Noccs+1;l++){
			dur_t_l =0.;
			pop_t_l =0.;
			for(d=0;d<Noccs;d++){
