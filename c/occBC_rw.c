/* This program solves my dissertation with rigid wages.  It follows Shimer (2012) in using steady state wages throughout the cycle.  When this implies a match
 * is not individualy rational, they do not occur.  Existing matches will renegotiate wages if they go outside of the Pareto efficient region
...
int main(int argc,char *argv[]){
	...
	*/
	...
	double lb_0[]	= {0.25	,0.0005		,0.005	,0.02		,0.005		,0.5	,-2.0			,-2.0			,-2.0	}; // what to do about sig_psi?  Skipping that right now
	double ub_0[]	= {0.8	,2.5		,0.15	,0.07		,0.20		,1.35	,.00			,.00			,.00	};
...
int sol_ss(gsl_vector * ss, gsl_vector * Zz, gsl_matrix * xss, struct sys_sol * sol){
	...
	gsl_vector * U_l0,*W_ld,*U_ld,J_ld;
	...
	U_l0 	= gsl_vector_calloc(Noccs+1);
	W_ld 	= gsl_vector_calloc(Noccs+Noccs*Noccs);
	U_ld 	= gsl_vector_calloc(Noccs+Noccs*Noccs);
	J_ld 	= gsl_vector_calloc(Noccs+Noccs*Noccs);
	...
	for(l=0;l<Noccs+1;l++){
		...
		//		gsl_vector_set(gld,l*Noccs+d,1.0);
			...
			U_l0->data[l] += gld->data[l*Noccs+d]*gsl_matrix_get(pld,l,d)*(chi[l][d] - bl);
	...
	for(iter=0;iter<itermax;iter++){
		...
		for(vfiter =0;vfiter<itermax/10;vfiter++){
					...
					if(l-1!=d){
						gsl_vector_set(W_ld,l*Noccs+d,
							((1.0-tau)*((1.0-sepld )*(chi[l][d]*exp(Z+zd) + beta*W_ld->data[l*Noccs+d])
									+ sepld*U_l0->data[0] ) + tau*W_ld->data[(d+1)*Noccs+d]  )
			...
			//				+ post
							...
							- bl - beta*U_l0->data[l] ;
					//ret  = (l == d+1) ? ret  - bl - beta*U_l0->data[l] : ret  - bl - beta*U_l0->data[0];
					ret *= (1.0-fm_shr);
					ret += bl + beta*U_l0->data[l];
					W_0 += gld->data[l*Noccs+d]*gsl_matrix_get(pld,l,d)*ret;
				}

				W_0 += (1.0-gsl_vector_get(findrt,l))*(bl + beta*U_l0->data[l]);
				//W_0 /= (1.0 - beta*(1.0-gsl_vector_get(findrt,l)));
				double distW0 = fabs(U_l0->data[l] - W_0)/U_l0->data[l];
				U_l0->data[l] = W_0;
				if(distW0 > maxdistW0)
					maxdistW0 = distW0;
			}
		...
		for(l=0;l<Noccs+1;l++){
			...
			for(d=0;d<Noccs;d++){
				...
				//double W0 	= l == d+1?  U_l0->data[l] : U_l0->data[0];
				double Wdif	= W_ld->data[l*Noccs+d] - U_l0->data[l] ;
			...
			for(d=0;d<Noccs;d++){
				...
				// sld
				double cutoff = -(chi[l][d]*exp(Z+zd) + beta*W_ld->data[l*Noccs+d] - U_l0->data[l]);
		...
		// this is actually testing the condition from U_l0 above
		if(break_flag ==1)
			break;
	...
	//	double wld_ld = ((1.0-fm_shr)*chi[l][d]*exp(Z+zd)- fm_shr*beta*(W_ld->data[l*Noccs+d] - U_l0->data[l] + bl-Exi)
	...
	//					/(1.0 + beta/(1.0-beta*(1.0-sld->data[l*Noccs+d])));
		double wld_ld = (1.0-fm_shr)*chi[l][d]*exp(Z+zd) + fm_shr*( Exi*(1.0-beta*(1.0-sld->data[l*Noccs+d]))
				+ bl + beta*U_l0->data[l] - beta*W_ld->data[l*Noccs+d])
...
//				double wld_ld = ((1.0-fm_shr)*chi[l][d]*exp(Z+zd)- fm_shr*beta*(W_ld->data[l*Noccs+d] - U_l0->data[l] + bl - Ephi)
...
//								/(1.0+ beta/(1.0-beta*(1.0-tau)*(1.0-sld->data[l*Noccs+d])));
	...
	// Steady state value functions Jld, Uld:
	for(d=0;d<Noccs;d++){// tenured guys
		gsl_vector_set(J_ld,(d+1)*Noccs+d,
				(1.0-sld->data[(d+1)*Noccs+d])*(chi[d+1][d]-gsl_matrix_get(sol->ss_wld,d+1,d))/(1.0- beta*(1.0-sld->data[(d+1)*Noccs+d]))
				);
		gsl_vector_set(U_ld,(d+1)*Noccs+d,
				(1.0-sld->data[(d+1)*Noccs+d])*(gsl_matrix_get(sol->ss_wld,d+1,d))/(1.0- beta*(1.0-sld->data[(d+1)*Noccs+d]))
				);
	}
...
//	gsl_eigen_nonsymmv_sort (eval, xxmat, GSL_EIGEN_SORT_ABS_DESC);
	...
	for(i=0;i<Noccs+1;i++){
		gsl_vector_set(ss,ssi,U_l0->data[i]);
		ssi++;	
	}
	...
	gsl_vector_free(U_l0);
	gsl_vector_free(W_ld);
	gsl_vector_free(U_ld);
	gsl_vector_free(J_ld);
	gsl_matrix_free(x);
	gsl_vector_free(gld);
	gsl_vector_free(sld);
...
//			double post = -kappa*gsl_matrix_get(sol->tld,l,d)/gsl_matrix_get(pld,l,d);
		...
		for(d=0;d<Noccs;d++){
			...
			double cont = Es->data[Wld_i+l*Noccs+d] - Es->data[Wl0_i+l];
			double fm_surp = (1.0-fm_shr)*chi[l][d]*exp(Z+zd) + gsl_matrix_get(sol->ss_wld,l,d) - fm_shr*(bl+ beta*cont);
			double cutoff= - (chi[l][d]*exp(Z+zd)+ Wdiff);
			cutoff = cutoff< -fm_surp ? fm_surp : cutoff;
			cutoff = cutoff >0.0 ? 0.0 : cutoff;
			double sep_ld = scale_s*exp(shape_s*cutoff);
			if(cutoff<0.0)
...
double bndCalMinwrap(double (*Vobj)(unsigned n, const double *x, double *grad, void* params),
	...
	if(alg0>=10 && alg0<100){//random restarts
		...
		printf("status of global solver is= %d\n", status);
		...
		fprintf(f_startx,"\n");
		fprintf(f_startf,"Genetic Algorithm Result!\n");
		fprintf(f_startf,"%f\n",opt_f);
		fprintf(f_startf,"\n");
		fclose(f_startx);
		fclose(f_startf);
