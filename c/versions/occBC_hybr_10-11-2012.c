...
gsl_matrix * pdf_occind; 			// pdf of occ-ind pairs
gsl_matrix * ind_zz; 				// history of zz_{industry} observed
...
int main(int argc,char *argv[]){
	...
	// read the anual history of shocks for the simulations
	ind_zz = gsl_matrix_alloc(Nt_dat,Nind);
	readmat("ilprod.csv",ind_zz);
...
int sol_ss(gsl_vector * ss, gsl_vector * Zz, gsl_matrix * xss, struct sys_sol * sol){
	...
	rete_d	= gsl_vector_calloc(Noccs);
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			for(ll=0;ll<2;ll++){
				...
				W_l0->data[l+ll*(Noccs+1)] += gld->data[l*Noccs+d+ll*JJ1]*gsl_matrix_get(pld,l+ll*(Noccs+1),d)*(chi[l][d] - (1.0-(double)ll)*bl);
			}
	...
	for(iter=0;iter<itermax;iter++){
		...
		for(vfiter =0;vfiter<itermax/2;vfiter++){
			...
			for(l=0;l<Noccs+1;l++){
				double findrt_ld =0.0;
				for(d=0;d<Noccs;d++)
					findrt_ld += gld->data[JJ1 + l*Noccs+d]*gsl_matrix_get(pld,l,d);
				gsl_vector_set(findrt,Noccs+1+l,findrt_ld);
			}
			// W_l0
			...
			for(l=0;l<Noccs+1;l++){
				...
				//				+ post
								...
								- bl*(1.0-(double)ll) - beta*((1.0-1.0/bdur)*W_l0->data[l + ll*(Noccs+1)]+1.0/bdur*W_l0->data[l+Noccs+1]);
						//ret  = (l == d+1) ? ret  - bl - beta*W_l0->data[l] : ret  - bl - beta*W_l0->data[0];
						double rete = ret + bl+ beta*(1.0-1.0/bdur)*(W_l0->data[l] -W_l0->data[l+Noccs+1]);
						ret 	*= (1.0-fm_shr);

						ret 	+= bl + beta*(1.0-1.0/bdur)*W_l0->data[l] + beta*1.0/bdur*W_l0->data[l+Noccs+1];

						W_0 	+= gld->data[l*Noccs+d]*gsl_matrix_get(pld,l,d)*ret;
					}

					W_0 	+= (1.0-gsl_vector_get(findrt,l))*(bl*(1.0-(double)ll) + beta*(1.0-1.0/bdur)*W_l0->data[l+ll*(Noccs+1)] + beta*1.0/bdur*W_l0->data[l+Noccs+1]);
		...
		for(l=0;l<Noccs+1;l++){
			...
			for(d=0;d<Noccs;d++){
				...
				for(ll=0;ll<2;ll++){
					...
					double arr_d = (1.0-fm_shr)*(chi[l][d]*(1.0+zd) -nud - (1.0-(double)ll)*bl + beta*Wdif)
							+ (1.0-(double)ll)*bl + (1.0-1.0/bdur)*W_l0->data[l+ ll*(Noccs+1)] + 1.0/bdur*W_l0->data[l + Noccs+1];
					if(ll==0)
						gsl_vector_set(ret_d,d,pld_ld*arr_d);
					else
						gsl_vector_set(rete_d,d,pld_ld*arr_d);
				}
			...
			double sexprete_dd =0.0;
			for(dd=0;dd<Noccs;dd++)
				sexprete_dd = gsl_vector_get(rete_d,dd)<= 0.0 ? sexprete_dd :
					exp(sig_psi*gsl_vector_get(rete_d,dd)) + sexprete_dd;
			...
			for(d=0;d<Noccs;d++){
				...
				gg = exp(sig_psi*gsl_vector_get(rete_d,d))/sexprete_dd;
				if(rete_d->data[d]<=0)
					gg =0;
				gsl_vector_set(gld,JJ1+l*Noccs+d,gg);
				...
				//Wdif = Wdif<0.0 ? 0.0 : Wdif;
				for(ll=0;ll<2;ll++){
					double Wdif	= W_ld->data[l*Noccs+d] - W_l0->data[ll*(Noccs+1)+l];
					double surp = chi[l][d]*(1.0+zd) - nud - (1.0-(double)ll)*bl +beta*Wdif;
					if(surp > 0.0){
						double qhere = kappa/(fm_shr*surp);
						gsl_vector_set(thetald,JJ1*ll+l*Noccs+d,invq(qhere) );
					}
					else
			...
			for(ll=0;ll<2;ll++){
				...
				if(gsum>0)
					...
					for(d =0 ;d<Noccs;d++)	gld->data[ll*JJ1 +l*Noccs+d] = 1.0/(double)Noccs;
			}
		}// for l=0:Noccs
		for(l=0;l<Noccs+1;l++){
			for(d=0;d<Noccs;d++){
		...
		for(l=0;l<Noccs+1;l++){
			for(ll=0;ll<2;ll++){
			for(d=0;d<Noccs;d++)
				gsl_matrix_set(pld,l+ll*(Noccs+1),d,pmatch(thetald->data[JJ1*ll + l*Noccs+d]) );
			}
		}
...
//								/(1.0+ beta/(1.0-beta*(1.0-tau)*(1.0-sld->data[l*Noccs+d])));
				double wld_ld = (1.0-fm_shr)*chi[l][d]*(1.0+zd) + fm_shr*(
						bl*(1.0-(double)ll) + beta*W_l0->data[l] - beta*W_ld->data[l*Noccs+d])
...
int sys_ex_set(gsl_matrix * N, gsl_matrix * S,struct shock_mats * mats){
	...
	gsl_matrix_view Lambdarf = gsl_matrix_submatrix(mats->Lambda,0,0,Noccs,Nfac*(Nllag+1));
	gsl_vector_view LambdarZ = gsl_matrix_column(mats->Lambda,Nfac*(Nllag+1));
...
int sys_st_diff(gsl_vector * ss, gsl_matrix * Dst, gsl_matrix * Dco, gsl_matrix* Dst_tp1, gsl_vector * xx){
	...
	for(l=0;l<Noccs+1;l++){
			...
			gsl_matrix_set(Dst_tp1,Wl0_i+l,Wl0_i+l +Noccs+1, beta*(1.0-findrtl)*1.0/bdur );
			...
			for(d=0;d<Noccs;d++){
				...
				for(ll=0;ll<2;ll++){
					...
					ss_ret += chi[l][d]*(1.0+zd) - bl*(1.0-(double)ll);
					...
					ss_ret += contval;
					ss_ret *= (1.0-fm_shr);
					ss_ret += bl*(1.0-(double)ll) + beta*(1.0-1.0/bdur)*ss->data[ss_Wl0_i+l + ll*(Noccs+1)] +
							beta*1.0/bdur*ss->data[ss_Wl0_i+l + Noccs+1];
					double u_ret = bl*(1.0-(double)ll) + beta*(1.0-1.0/bdur)*ss->data[ss_Wl0_i+l + ll*(Noccs+1)]+
							beta*1.0/bdur*ss->data[ss_Wl0_i+l + Noccs+1];
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(ll=0;ll<2;ll++){
			...
			for(d=0;d<Noccs;d++){
				...
				double pld = pmatch(ss->data[ss_tld_i+l*Noccs+d + JJ1*ll]);
				ret_d->data[d] = (1.0-fm_shr)*(chi[l][d]*(1.0+zd)-bl+nud+beta*contval)
						+ bl + beta*( (1.0-1.0/bdur)*ss->data[ss_Wl0_i+l +ll*(Noccs+1)]+(1.0/bdur)*ss->data[ss_Wl0_i+l +Noccs+1]);
			...
			// tld
				...
				if(tld_ss>0.0){
					...
					double surp = chi[l][d] - nud - bl*(1.0-(double)ll) + beta*(ss->data[ss_Wld_i+l*Noccs+d]-W_l0);
					//dt/dWld
					double qhere = kappa/(fm_shr*surp);
				...
				if(gld_ss>0 && tld_ss>0){
					...
					double arr_d 	= (1.0-fm_shr)*(chi[l][d]*(1.0+zd) -bl -nud + beta*contval)
										+ bl + (1.0-1.0/bdur)*ss->data[ss_Wl0_i+l+ll*(Noccs+1)]+
										1.0/bdur*ss->data[ss_Wl0_i+l+Noccs+1];
				...
				if(dd!=d){
					if(ss->data[ss_gld_i+l*Noccs+d]>0.0 && ss->data[ss_tld_i+l*Noccs+dd+ll*JJ1]>0.0){
						...
						double arr_dd 	= (1.0-fm_shr)*(chi[l][dd]*(1.0+zdd)-bl - nudd+beta*contval)
											+ bl + (1.0-1.0/bdur)*ss->data[ss_Wl0_i+l+ll*(Noccs+1)]+
											1.0/bdur*ss->data[ss_Wl0_i+l+Noccs+1];
	...
	for (l=0;l<Noccs+1;l++){
		...
		for(ll=0;ll<2;ll++){
			...
			for(d=0;d<Noccs;d++){
				...
				double nud 	= l ==d+1? 0.0 : nu;
				double ret_d= (1.0-fm_shr)*(chi[l][d]-bl- nud +
		...
		//			);
			for(d=0;d<Noccs;d++){
				...
				double nud = l ==d+1? 0.0 : nu;
				double pld 	= pmatch(ss->data[ss_tld_i+l*Noccs+d + ll*JJ1]);
				double post	= pld>0 ? - kappa*ss->data[ss_tld_i+l*Noccs+d + ll*JJ1]/pld : 0.0;
									// dWl0/dz
				double dWdz =ss->data[ss_gld_i+l*Noccs+d + ll*JJ1]*(1.0-fm_shr)*chi[l][d]*pld;
				...
				if(ss->data[ss_tld_i+l*Noccs+d+ ll*JJ1]>0.0){
					double surp = (chi[l][d]-nud- bl*(1.0-(double)ll) +beta*(ss->data[ss_Wld_i+l*Noccs+d]-(1.0-1.0/bdur)*ss->data[ss_Wl0_i+l + ll*(Noccs+1)] -
							1.0/bdur*ss->data[ss_Wl0_i+l + Noccs+1]));
...
int sol_zproc(struct st_wr *st, gsl_vector * ss, gsl_matrix * xss){
	...
	gsl_matrix * tzhist	= gsl_matrix_alloc(simT,Noccs);
	...
	for(estiter=0;estiter<maxestiter;estiter++){
		...
		for(di=0;di<simT;di++){
			...
			gsl_vector_memcpy(Zz,Zzl);
			if(estiter==0){
				gsl_vector_view zd = gsl_vector_subvector(Zz,Notz,Noccs);
				gsl_matrix_set_row(simdat->tildez,di,&zd.vector);
				gsl_vector_set(Zhist,di,Zz->data[0]);
			}
			if(fix_fac==1){
				for(d=0;d<Nfac;d++)
			...
			for(d=0;d<Noccs;d++){
				...
				for(l=0;l<Noccs+1;l++){
					zd 	+= chi[l][d]*gsl_matrix_get(xp,l,d+1);
					xd	+=  gsl_matrix_get(xp,l,d+1);
				}
				double xdozd = xd > 0.0 ? xd/zd : 1.0;
				gsl_matrix_set(zhist,di,d,gsl_matrix_get(simdat->tildez,di,d)*xdozd);
				avg_scale += xd/zd;
			}
		...
		double scale_var_zeta = (1.0-rhozz*rhozz)/(1.0-mats1.rhozz);
	...
	gsl_matrix_free(tzhist);gsl_matrix_free(zhist);gsl_matrix_free(zhist0);
		...
		*/
		...
		// compute wages in  this period
			...
			//				bEJ - fm_shr*beta*(Es->data[Wld_i + l*Noccs+d] -Es->data[Wl0_i+l]) - fm_shr*(bl+Exi);
					double wld_ld = (1.0-fm_shr)*chi[l][d]*(1.0+Zz->data[Notz+d]) +
							bEJ - fm_shr*beta*(Es->data[Wld_i + l*Noccs+d] -Es->data[Wl0_i+l]) - fm_shr*(bl+Exi);
	...
	*/
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			double ret_d 	= (1.0-fm_shr)*(chi[l][d]*(1.0 + zd) - bl - nud + beta*cont)
									+ bl + Es->data[Wl0_i + l];
...
int est_fac_pro(gsl_matrix* occ_prod,gsl_vector* mon_Z, struct shock_mats * mats){
	...
	double rhozz	= gsl_stats_correlation (vzz->data, vzz->stride, vzztm1->data, vzztm1->stride, vzztm1->size);
	for(t=0;t<T;t++){
		for(c=0;c<Noccs;c++){
	...
	for(iter=0;iter<maxiter;iter++){
		if(fix_fac!=1){
			status += pca(residzz,Nfac,1,loadings,facs);
			double load_scale = 0.0,load_mean=0.0;
			for(c=0;c<Noccs;c++)
				load_mean += gsl_matrix_get(loadings,c,0)/(double)Noccs;
			for(c=0;c<Noccs;c++)
				load_scale += pow(gsl_matrix_get(loadings,c,0)-load_mean,2)/(double)Noccs;
			load_scale = sqrt(load_scale);
			gsl_matrix_scale(facs,load_scale/lambda_scale);
		}
		else{
...
*/
	...
	(st->sim)->tildez			= gsl_matrix_alloc(simT,Noccs);
