...
double 	kappa	= 0.35;//.1306; corresponds to HM number
double 	phi		= .4;//0.455997;	// : log(1/UE elasticity -1) = phi*log(theta)= log(1/.38-1)/log(5)
double 	fm_shr	= 0.7;	// firm's surplus share
double 	effic	= .99;//1.162658;
...
//				dtld = surp<0 ? 0.0 : dtld;
			...
			if(gld_ss>0){
				...
				*/
				...
				double dpdt = effic/pow(1.0 + pow(tld_ss,phi),1.0/phi +1.0);
