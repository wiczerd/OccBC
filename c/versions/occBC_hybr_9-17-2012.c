...
double sig_eps	= 6.73e-7;//1.2610e-06;//0.00008557;//0.00002557;//7.2445e-7;  using mine, not FED Numbers
...
int sim_moments(struct st_wr * st, gsl_vector * ss,gsl_matrix * xss){
	...
	if(status == 0 && finmoments!=0){
		...
		if(st->cal_set !=3){
			...
			gsl_matrix_set(simdat->data_moments,0,0,0.0);
			gsl_matrix_set(simdat->data_moments,0,2,0.0);
		}
...
int TGR(struct st_wr * st){
	...
	for(l=1;l<Noccs+1;l++){
		...
		for(ll=0;ll<Noccs+1;ll++){
			if(ll!=l)
				gsl_matrix_set(x,d,l,(1.0-frdd)*xll_t*gsl_matrix_get(st->xss,ll,l)/xld_av);
		}
	}
	// check the sum:
	if(printlev>=2)
...
int est_fac_pro(gsl_matrix* occ_prod,gsl_vector* mon_Z, struct shock_mats mats){
	gsl_matrix * zz, *residzz;
	gsl_vector *Ztm1, *Z, *vzztm1,*vzz,*vresidzz,*coefregs,*lvresidzz;
	gsl_matrix * facs, *loadings,*xreg;
	int t,c,fi,status = 0;
	int T = mon_Z->size-1;
	int iter,maxiter=500;
	facs 	= gsl_matrix_alloc(T,Nfac);
	loadings = gsl_matrix_alloc(Nfac,Noccs);
	...
	for(t=1;t<T+1;t++)
		sig_eps += pow(gsl_vector_get(mon_Z,t)-rhoZ*gsl_vector_get(mon_Z,t-1),2)
						/((double)T);
	status += gsl_matrix_set_col(xreg, Noccs*(Nfac+1)-1, vzztm1);
	...
	for(iter=0;iter<maxiter;iter++){
		for(c=0;c<Noccs;c++){
			...
			for(fi=0;fi<Nfac;fi++){
				...
				status += gsl_matrix_set_col(&xreg_c.matrix,fi+1,&facs_i.vector);
			}
			gsl_vector_view Z_sub = gsl_vector_subvector(mon_Z,1,T);
			status += gsl_matrix_set_col(xreg, Nfac+1, &Z_sub.vector);
		}
		...
		for(c=0;c<Noccs;c++){
			for(t=0;t<T;t++)
				gsl_matrix_set(residzz,t,c, gsl_vector_get(vzz,c*T+t)-gsl_vector_get(coefregs,Noccs*(Nfac+1)-1)*gsl_vector_get(vzztm1,c*T+t)
