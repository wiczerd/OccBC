...
int printlev 	= 3;
int use_anal	= 1;
int opt_alg		= 0; // 10x => Nelder-Meade, 5x => Subplex, o.w => BOBYQA
int polish_alg	= 0;
int homosk_zeta	= 0;
int diag_zeta	= 1;
int gsl_fin_diffs=0;
...
/*
#ifdef _MKL_USE
	gsl_matrix * F0LU =gsl_matrix_calloc(Nc,Nc);
...
double bndCalMinwrap(double (*Vobj)(unsigned n, const double *x, double *grad, void* params),
	...
	int status,resi,starts,i,locstatus,csize;
	status 	= 0;
	csize 	= 32; // how many nodes it's running on.
	starts 	= 150*csize; // this is how many will draw to begin, but will only start from a subset of these
...
//  Non- deriv based
	...
	if(alg0>=10){//random restarts with MPI
		double * dim_scale = malloc(sizeof(double)*n);
		for(i=0;i<n;i++){
			dim_scale[i] = gsl_vector_get(ub,i) - gsl_vector_get(lb,i);
		}
		...
		for(resi=0;resi<starts;resi++)
			x0_loc = (double*)malloc(sizeof(double)*n);
		double d_inner=0.0,d_outer=1000.0; // take everyone to start
		int stop_crit_N=starts;
		double d_region[csize];
		for(resi=0;resi<starts;resi++){
			// update criteria for how many points we expect and for size of region of attraction each csize times
			int li;
			if(resi>0 && resi%csize==0){
				d_inner = d_outer;
				for(li=0;li<cszie;li++)
					d_inner = d_inner>d_region[li] ? d_region[li] : d_inner;
				stop_crit_N = (nmax*(resi-1))/(resi-nmax-2)-0.5;
				// the whole region is size 1, so this is narrowing in at rate sqrt(n/clustersize)
				d_outer = 1.0/ sqrt( resi/csize);
			}
			...
			if((d_xres>d_inner && d_xmin<d_outer) || resi%5==0){
				...
				d_region[resi%csize] = 0.0;
				for(i=0;i<n;i++)
					d_region[resi%csize] += fabs(xstart[i] - x0queue[resi][i])/dim_scale[i] ;

				// test if this is a new optim location
				double d_xres=10.0;	int li;
				for(li=0;li<nmax;li++){
					double d_xres_li = 0.0;
					for(i=0;i<n;i++)
						d_xres_li += fabs(x0_loc[li][i]-x0queue[resi][i])/dim_scale[i] ;
				...
				if(d_xres<1e-5){// close enough to call the same as one of these points
					for(i=0;i<n;i++)
