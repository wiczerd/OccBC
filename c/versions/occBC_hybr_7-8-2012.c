...
double 	sig_psi	= .72865434;//3.671249; 	// higher reduces the switching prob

double *	b; // unemployment benefit


double ** chi;
double 	tau 	= 0.016561;//0.020659;
double 	kappa	= 0.26;//.1306; corresponds to HM number
double 	phi		= .45;//0.455997;	// : log(1/UE elasticity -1) = phi*log(theta)= log(1/.38-1)/log(5)
double 	fm_shr	= 0.66;	// firm's surplus share
double 	effic	= 1.0;//1.162658;
double 	sbar	= 0.02;	// will endogenize this a la Cheremukhin (2011)

double 	scale_s	= 0.025;
double 	shape_s	= 0.00010;
...
int sim_moments(struct st_wr * st, gsl_vector * ss,gsl_matrix * xss){
	...
	gsl_matrix * uf_hist, * urt_l, * fnd_l_hist, * x_u_hist, * Zzl_hist, * s_wld;
	if(printlev>=2){
		uf_hist		= gsl_matrix_calloc(Ndraw,3);
	...
	if(printlev>=2){
		gsl_matrix_free(uf_hist);gsl_matrix_free(Zzl_hist);
...
int theta(gsl_matrix * tld, const gsl_vector * ss, const struct sys_coef * sys, const struct sys_sol * sol, const gsl_vector * Zz){
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			double qhere = kappa/(fm_shr*surp) > effic ? effic : kappa/(fm_shr*surp);
...
void dfovec_iface_(double * f, double * x, int * n){
	...
	int i;
	double *grad = NULL;
	double * x0 = (double*)malloc(sizeof(double)*nv);
	for(i=0;i<nv;i++)
...
double bndCalMinwrap(double (*Vobj)(unsigned n, const double *x, double *grad, void* params),
	...
	if(dfbols_flag == 1){
		...
		w = (double*)malloc(sizeof(double)*nspace);
		// rescale bounds in the x dimension, this is always going to be unit-box bounded
		dfbols_lb = (double*)malloc(sizeof(double)*n);
		dfbols_ub = (double*)malloc(sizeof(double)*n);
		for(i=0;i<n;i++){
