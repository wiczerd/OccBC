...
double med_dr	= 13;
double chng_pr	= 0.45;
double sk_wg[]	= {-0.610,-0.631,-0.400};//{0.0173749,.0462705,-0.03424,0.2024};//{0.0117762,-0.0351453,0.01041,0.2074417}; //abs 1 - 3
...
int main(int argc,char *argv[]){
	...
	*/
	...
	double lb_0[]	= {0.15	,0.001		,0.001	,0.015		,0.000001	,0.1,	-.5		,-.5	,-.5	}; // what to do about sig_psi?  Skipping that right now
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			if(l-1==d){
				...
				Exi = scale_s*((1.0/shape_s+barxi)*exp(-shape_s*barxi)-1.0/shape_s);
			}
			double bEJ = beta*fm_shr*(W_ld->data[l*Noccs+d] - W_l0->data[l]);  // fill this in with shr*E(surp)::: not sure if this is correct

			double wld_ld = (1.0-fm_shr)*chi[l][d]+
					bEJ - fm_shr*beta*(W_ld->data[l*Noccs+d] - W_l0->data[l]) + fm_shr*(bl-Exi);
...
int sim_moments(struct st_wr * st, gsl_vector * ss,gsl_matrix * xss){
	...
	for(di = 0;di<Ndraw;di++){
		...
		// only look at experienced changers, and set chi[0][d] s.t. makes the avg wage loss match
			...
			if(printlev>=4){
				printmat("XXloss.csv",XX);
				printmat("Wt.csv",Wt);
				printvec("yloss.csv",yloss);
			}
			...
			gsl_matrix * Wc = gsl_matrix_alloc(Xrow,Xrow);gsl_matrix_memcpy(Wc,&W_v.matrix);
			gsl_matrix * Xc = gsl_matrix_alloc(Xrow,XX->size2);gsl_matrix_memcpy(Xc,&X_v.matrix);
			gsl_vector * yc = gsl_vector_alloc(Xrow);gsl_vector_memcpy(yc,&y_v.vector);
			if(printlev>=4){
