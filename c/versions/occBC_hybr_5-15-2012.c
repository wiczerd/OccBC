...
int sol_ss(gsl_vector * ss, gsl_matrix * xss, struct sys_coef * sys){
	...
	maxtss = pow(0.99/effic,1.0/(1.0-phi));
	maxtss = maxtss < pow(0.99/effic,1.0/(-phi)) ? maxtss : pow(0.99/effic,1.0/(-phi));
		...
		for(l=0;l<Noccs+1;l++){
			...
			for(d=0;d<Noccs;d++){
				...
				if(tinner > effic*pow(mintss,1.0-phi) )
					theta =	pow(effic*fm_shr/kappa,1.0/phi)*
						pow(tinner,1.0/phi) ;
...
int sys_st_diff(gsl_vector * ss, gsl_matrix * Dst, gsl_matrix * Dco, gsl_matrix* Dst_tp1, gsl_vector * xx){
	...
	*/
			...
			for(d=0;d<Noccs;d++){
				...
				double dtld =effic*(1.0-phi)*pow(ss->data[ss_tld_i+l*Noccs+d],-phi)
				...
				double disc_cont =beta*effic*pow(ss->data[ss_tld_i+l*Noccs+d],1.0-phi)*ss->data[ss_gld_i+l*Noccs+d];
...
int sys_co_diff(gsl_vector * ss, gsl_matrix * Dst, gsl_matrix * Dco, gsl_matrix* Dst_tp1, gsl_vector * xx){
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			//dt/dWld
			double dtld =(beta*fm_shr*effic/phi/kappa)*pow(tinside,1.0/phi-1.0);
			if(gsl_finite(dtld))
				gsl_matrix_set(Dst_tp1,tld_i + l*Noccs+d,Wld_i + l*Noccs+d, dtld);
			else
				gsl_matrix_set(Dst_tp1,tld_i + l*Noccs+d,Wld_i + l*Noccs+d, 0.0);
			//dt/dWl0
			dtld = -(beta*fm_shr*effic/phi/kappa)*pow(tinside,1.0/phi-1.0);
			if(gsl_finite(dtld))
			...
			*/
			// dg/dt
			double dexret_d = sig_psi*exp(sig_psi*ret_d)*( (1.0-phi)*effic*pow(ss->data[ss_tld_i+l*Noccs+d],-phi)*(arr_d)-kappa*phi );
			...
			// IS THIS THE PROBLEM???
				if(ss->data[ss_tld_i+l*Noccs+dd]>mintss && arr_dd>0.0)
					gsl_matrix_set(Dco,gld_i+l*Noccs+d,tld_i+l*Noccs+dd,
						-1.0*
						-sig_psi*exp(sig_psi*ret_dd)*((1.0-phi)*effic*pow(ss->data[ss_tld_i+l*Noccs+dd],-phi)*(arr_dd)
						-kappa*phi )*exp(sig_psi*ret_dd) /pow(gdenom,2)
					);
				else
