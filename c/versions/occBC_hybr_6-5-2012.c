...
*  rev: 5-06-2012
*
* compile line: icc occBC_hybr.c -static -lgsl -openmp -mkl -lnlopt -I/home/david/Computation -I/home/david/Computation/gsl/gsl-1.14 -o occBC_hybr.out
*/
...
double 	phi		= 0.65;//0.4;	// : log(1/UE elasticity -1) = phi*log(theta)= log(1/.38-1)/log(5)
...
*/
	...
	double lb_0[]	= {0.15,0.05,0.0001,0.1,x0_0[3]*(0.7),x0_0[4]*(0.7),x0_0[5]*(0.7),x0_0[6]*(0.7)}; // what to do about sig_psi?  Skipping that right now
	double ub_0[]	= {1.0,15.0,0.05,1.5,x0_0[3]*(1.3),x0_0[4]*(1.3),x0_0[5]*(1.3),x0_0[6]*(1.3)};
 ...
 */
	...
	status += isfinmat(sol->P0);
	status += isfinmat(sol->P1);
	status += isfinmat(sol->P2);
...
//	Wage growth
	...
	if(status ==0){
		...
		gsl_matrix_set_all(simdat->data_moments,100);
...
double cal_dist(unsigned n, const double *x, double *grad, void* params){
	...
	*/
	...
	if(st->cal_set==0 || st->cal_set==2){
		...
		if(printlev>=1){
			calhist = fopen("calhist.txt","a");
			fprintf(calhist,"dist=%f, phi=%f,sig_psi=%f,tau=%f,ef=%f\n",dist,x[0],x[1],x[2],x[3]);
			fprintf(calhist,"d_1 = %f, d_2=%f, d_3=%f, d_4=%f\n",
					(simdat->data_moments)->data[0],(simdat->data_moments)->data[1],(simdat->data_moments)->data[2],(simdat->data_moments)->data[3]);
			fclose(calhist);
		}
	}
