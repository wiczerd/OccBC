	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			*/
			...
			// IS THIS THE PROBLEM???
				if(ss->data[ss_tld_i+l*Noccs+d]>mintss+5e-6 && arr_dd>0.0)
...
// derivatives for exog
	...
	for (l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			double tinside = (chi[l][d]-nud- bl +beta*(ss->data[ss_Wld_i+l*Noccs+d]-ss->data[ss_Wl0_i+l]))
					*effic*fm_shr/kappa;
			tinside = tinside<0 ? chi[l][d]*effic*fm_shr/kappa : tinside;
			gsl_matrix_set(Dco,tld_i + l*Noccs+d,0,   // dtheta/dZ
				(1.0/phi)*pow(tinside,1.0/phi-1.0)*chi[l][d]*effic*fm_shr/kappa
				);
