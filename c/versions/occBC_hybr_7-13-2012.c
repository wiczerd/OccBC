...
double 	sig_psi	= 0.507194; 	// higher reduces the switching prob
...
double ** chi;
double 	tau 	= 0.009450;
double 	kappa	= 0.26;//.1306; corresponds to HM number
double 	phi		= 0.442140;	// : log(1/UE elasticity -1) = phi*log(theta)= log(1/.38-1)/log(5)
double 	fm_shr	= 0.66;	// firm's surplus share
double 	effic	= 1.099987;
double 	sbar	= 0.02;	// will endogenize this a la Cheremukhin (2011)

double 	scale_s	= 0.034141;
double 	shape_s	= 0.063135;
...
double med_dr	= 13;
double chng_pr	= 0.45;
double sk_wg[]	= {-0.610,-0.631,-0.399};//{0.0173749,.0462705,-0.03424,0.2024};//{0.0117762,-0.0351453,0.01041,0.2074417}; //abs 1 - 3
...
int main(int argc,char *argv[]){
	...
	for(l=0;l<f_skills->size1;l++){
		double occ_sum =0.0;
		for(i=1;i<Nskill+1;i++)
			occ_sum += gsl_matrix_get(f_skills,l,i);
		for(i=1;i<Nskill+1;i++)
			gsl_matrix_set(f_skills,l,i,gsl_matrix_get(f_skills,l,i)/occ_sum);
	}
	...
	if(homosk_zeta==1){
		...
		gsl_matrix_scale(var_zeta,3.1144); // adjust so that has the same unconditional variance as with the true persistence
	}
	else
		readmat("var_zeta.csv",var_zeta);
	...
	*/
	...
	double lb_0[]	= {0.15	,0.001		,0.001	,0.015		,0.000001	,0.1,	-2.5			,-2.5			,-2.5	}; // what to do about sig_psi?  Skipping that right now
	double ub_0[]	= {0.67	,2.5		,0.03	,0.04		,0.20		,1.15,	.01				,.01			,.01};
...
int sol_ss(gsl_vector * ss, gsl_matrix * xss, struct sys_sol * sol){
	...
	for(iter=0;iter<itermax;iter++){
		...
		for(vfiter =0;vfiter<itermax/10;vfiter++){
			...
			if(maxdistW0<W0_tol)// not particularly precise
				break;
...
*/
		...
		for(l=0;l<Noccs+1;l++){
			...
			for(d=0;d<Noccs;d++){
				...
				// = w+ beta E[W_ld']
				//double arr_d = (1.0 - fm_shr)*(chi[l][d] -nud - bl + post + beta*Wdif)
				//		+ bl + beta*W_l0->data[l];
		...
		if(break_flag ==1)
			...
			break_flag=1;
			printf("Not making progress in SS from %f\n",maxdistg);
		}
		lmaxdistg = maxdistg;
		if(printlev>=4){
...
int sim_moments(struct st_wr * st, gsl_vector * ss,gsl_matrix * xss){
	...
	gsl_matrix * urt_l, * fnd_l_hist, * x_u_hist, * Zzl_hist, * s_wld;
	if(printlev>=2){
			...
			for(l=1;l<Noccs+1;l++){
				for(d=0;d<Noccs;d++){
					if(l-1!=d){
						...
						//double ylossld = log(chi[l][d]/chi[d+1][d]);
						double wgt = pld[l][d]*gsl_matrix_get(sol->gld,l,d)*x_u->data[l]/(1.0-x_u->data[0]);
						if(gsl_finite(ylossld) && ylossld<0.0 && wgt>0.0 && gsl_finite(wgt)){
	...
	if(verbose>=3){
		printf("wage reg: ");
		for(di=0;di<Nskill;di++)printf("%f,",coefs->data[di]- sk_wg[di]);
	}
	// Cleaning up!
	// !!!!!!!!!!!!
	...
	gsl_vector_free(fnd_l);gsl_vector_free(Zzl);gsl_vector_free(shocks);gsl_matrix_free(uf_hist);
	if(printlev>=2){
	...
	gsl_matrix_free(x);gsl_matrix_free(xp);gsl_vector_free(Zz);gsl_matrix_free(wld);
	return status;
}
...
int endog_std(double stdz, double stdZ,gsl_matrix * x0, struct st_wr * st){
	int status=0,l;
	// stdz scales the diagonal of the E[zeta zeta']

	gsl_matrix * xp = gsl_matrix_alloc(Noccs+1,Noccs+1);
	gsl_vector * Zzpstd = gsl_vector_calloc(Nx);
	Zzpstd->data[0] = sig_eps*stdZ;
	xprime(xp, st->ss, st->sys, st->sol, x0, Zzpstd);
	// save this distribution
	for(l=0;l<Noccs;l++){
		Zzpstd->data[l+Notz] = gsl_matrix_get((st->sys)->S,l+Notz,l+Notz)*stdz;
		xprime(xp, st->ss, st->sys, st->sol, x0, Zzpstd);
		// save this distribution
	}
	//compute the standard deviation of total productivity
