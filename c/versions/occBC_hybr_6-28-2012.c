...
int printlev 	= 3;
int use_anal	= 1;
int opt_alg		= 15; // 10x => Nelder-Meade, 5x => Subplex, o.w => BOBYQA
int polish_alg	= 0;
...
*/
	...
	*/
	...
	double ub_0[]	= {1.0,10.0,0.05,0.03, 0.5,1.5,sk_wg[0]*(1.3),sk_wg[1]*(1.3),sk_wg[2]*(1.3),sk_wg[3]*(1.3)};
 ...
 */
	...
	//status += LAPACKE_dgetrs(LAPACK_ROW_MAJOR, 'N',  (sys->F0)->size1, (sys->F0)->size2, F0LU, (sys->F0)->size2, ipiv, invF0F1, Ns);
...
double bndCalMinwrap(double (*Vobj)(unsigned n, const double *x, double *grad, void* params),
	...
	starts 	= 15*csize; // this is how many will draw to begin, but will only start from a subset of these
