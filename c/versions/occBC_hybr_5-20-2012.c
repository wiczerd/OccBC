...
double 	sig_psi	=10.627577; // higher reduces the switching prob
...
double ** chi;
double 	tau 	= 0.0041;//0.003857;
double 	kappa	= .1338;
double 	phi		= 0.4;	// match elasticity
	...
	*/
	...
	for(l=1;l<Noccs+1;l++){
		for(d=0;d<Noccs;d++){
			chi[l][d] = 0.0;
			for(i=0;i<Nskill;i++){
			//	double d_chi_ld =sk_wg[i]*gsl_matrix_get(f_skills,d,i+1) + sk_wg[Nskill+i]*gsl_matrix_get(f_skills,l-1,i+1);
...
*/
		...
		for(l=0;l<Noccs+1;l++){
			...
			for(d=0;d<Noccs;d++){
				...
				double theta =pow(pow(kappa
						/(effic*fm_shr*tinner),-phi)-1.0,1.0/phi) ;
				if(tinner > 0.0 && gsl_finite(theta) )
...
int sim_moments(struct aux_coef* simdat,gsl_vector * ss,gsl_matrix * xss,struct sys_sol * sol,struct sys_coef * sys){
	...
	for(di = 0;di<Ndraw;di++){
		...
		double d_chng = 0.0;
		for(l=1;l<Noccs+1;l++){
			double d_chng_l = 1.0 - (gsl_matrix_get(sol->gld,l,l-1)*pld[l][d] )/
				fnd_l->data[l];
...
//	Wage growth
		...
		for(l=0;l<Noccs+1;l++){
			for(d=0;d<Noccs;d++){
				d_wg += gsl_vector_get(x_u,l)*pld[l][d]*(chi[d+1][d] - chi[l][d]);
			}
		}
		...
		for(l=1;l<Noccs+1;l++){
			for(d=0;d<Noccs;d++){
				if(l!=d+1){
					d_wl +=gsl_vector_get(x_u,l)*pld[l][d] *(chi[l][l-1] - chi[l][d]);
					sx_wl +=gsl_vector_get(x_u,l)*pld[l][d];
				}
			}
		}
		d_wl /=sx_wl;
	...
	*/
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			double post = gsl_matrix_get(pld,l,d)>0 ? -kappa*gsl_matrix_get(sol->tld,l,d)/gsl_matrix_get(pld,l,d) : 0.0;
			double ret_d =chi[l][d]*exp(Z + zd) - bl - nud +post + beta*(Es->data[Wld_i + l*Noccs+d] -Es->data[Wl0_i + l] ) ;
			double gld_ld 	=exp(sig_psi*gsl_matrix_get(pld,l,d)*ret_d);
			gld_ld 	= gsl_matrix_get(pld,l,d)*ret_d <= 1e-8 ? 0.0 : gld_ld;
			gld_ld 	= gld_ld < 0.0 ? 0.0 : gld_ld;
			gld_ld 	= gsl_finite(gld_ld) == 0 ? 0.0:gld_ld;
			gld_l[d]= gld_ld;
			gdenom += gld_ld;
		}
		if(gdenom>0){
...
int theta(gsl_matrix * tld, const gsl_vector * ss, const struct sys_coef * sys, const struct sys_sol * sol, const gsl_vector * Zz){
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			double nud = l == d+1? 0:nu;
			double zd = Zz->data[d+1+Ntfpl+Nfac*(Nflag)];
			double surp =chi[l][d]*exp(Z+zd) - bl - nud + beta*(Es->data[Wld_i+l*Noccs+d] - Es->data[Wl0_i+l]);
			...
			tld_i = tld_i>0.0? tld_i : 0.0;
			tld_i = inside >0.0 ? tld_i : 0.0;
...
//	b		= x[6];
	...
	for(l=1;l<Noccs+1;l++){
		for(d=0;d<Noccs;d++){
			chi[l][d] = 0.0;
			for(i=0;i<Nskill;i++){
				chi[l][d] += x[3+i]*fabs(gsl_matrix_get(f_skills,d,i) - gsl_matrix_get(f_skills,l-1,i));
			}
			chi[l][d] += x[3+Nskill];
		}
	}
