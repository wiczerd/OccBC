...
*  First, it solves the first order approximation by QZ decomposition.  The 2nd order is
*  computed following Lombard & Sutherland
...
*  and one choice variable, i.e. capital and endogeneous hours, and any number of shocks.  In
...
double 	sig_psi	= .10;	// sd of taste shocks
...
double 	effic	= 0.25;
double 	sbar	= 0.01;	// will endogenize this a la Cheremukhin (2011)

double rhoZ		= 0.9;
double rhozz	= 0.9;
double sig_eps	= 0.01;
double sig_zet	= 0.01;
double sig_fac[]= {0.01,0.01 };
double * corrZ;
...
int dur_dist(gsl_vector * fnd_dist, gsl_vector * dur_dist, const gsl_vector * fnd_l, const gsl_vector * x_u);
...
int main(){
	...
	double sk_wg[] = {.02200879,  -.01494673,   .04459289,  -.01431361, .1641782};
	int Nskill = 4;
	// with a mean:
	 ...
	 *
	*/
	gsl_matrix * f_skills = gsl_matrix_alloc(Noccs,6);
...
*/
	...
	simdat.data_moments = gsl_matrix_alloc(5,1); // pick some moments to match
...
/*	simdat.data_moments->data[0] = avg_sep;
	...
	nu		= x[1];
	tau 	= x[2]; */
	double lb[]	= {0.005,0.0,0.005}; // what to do about sig_psi?  Skipping that right now
	double ub[]	= {1.0,0.45,0.5};
	double x0[]	= {effic,nu,tau};
	...
	double objval = bndCalMinwrap(&cal_dist,lb,ub,3,x0,&st,1e-7,1e-7,0.0,0);
 ...
 * The technique follows page 3 of my Leuchtturm1917
 *
 */
int sol_dyn(gsl_vector * ss, struct sys_sol * sol, struct sys_coef *sys){
		...
		for(l=0;l<Noccs+1;l++){
			...
			if(l-1!=d){
				gsl_vector_set(W_ld,l*Noccs+d,
					((1.0-tau)*((1.0-sbar)*chi[l][d]+sbar*W_l0->data[l] ) + tau*(1.0-sbar)*W_ld->data[(d+1)*Noccs+d]  )
						/(1.0-(1.0-sbar)*(1.0-tau)*beta));
			}
			}
		}
		if(printlev>=2 && iter==0) printvec("W_ld_0.csv",W_ld);
		if(printlev>=2 && iter >0) printvec("W_ld_i.csv",W_ld);
		// update policies
		...
		for(l=0;l<Noccs+1;l++){
			for(d=0;d<Noccs;d++){
				...
				gsl_vector_set(ret_d,d,
					gsl_matrix_get(pld,l,d)*(chi[l][d] -nud - b + beta*(W_ld->data[l*Noccs+d] - W_l0->data[l]))
				);
			}
			...
			for(d=0;d<Noccs;d++){
				...
				theta = theta > pow(1.0/effic,1.0/(1.0-phi)) ? pow(1.0/effic,1.0/(1.0-phi)) : theta;
...
//	int wld_i, tld_i, x_i,gld_i;
	...
	if(verbose >=2)
	printf("Sz1 A1=%d,Sz2 A1=%d, Sz1 A2=%d, Sz2 A2=%d\n",
			(int) (sys->A1)->size1,(int)(sys->A1)->size2,(int)(sys->A2)->size1,(int)(sys->A2)->size2);
...
int sys_st_diff(gsl_vector * ss, gsl_matrix * Dst, gsl_matrix * Dco, gsl_matrix* Dst_tp1, gsl_vector * xx){
	...
	for(l=0;l<Noccs+1;l++){
			...
			for(d=0;d<Noccs;d++){
				double nud = l ==d+1? 0.0 : nu;
				gdenom +=exp(sig_psi*effic*pow(ss->data[ss_tld_i+l*Noccs+d],1.0-phi)*(chi[l][d]-b+nud+beta*(ss->data[ss_Wld_i+l*Noccs+d]-ss->data[ss_Wl0_i+l])));
			}
	/*		// xl0
	...
	*/
			...
			for(d=0;d<Noccs;d++){
				...
				// Wl0
				double ss_ret = d+1==l ? chi[l][d]*exp(Z+zd) -b +beta*(ss->data[ss_Wld_i+l*Noccs+d] -ss->data[ss_Wl0_i+l] ) :
					-b -nu +beta*(ss->data[ss_Wld_i+l*Noccs+d] -ss->data[ss_Wl0_i+l] );
				gsl_matrix_set(Dco,Wl0_i+l,tld_i+l*Noccs+d,
						effic*(1.0-phi)*pow(ss->data[ss_tld_i+l*Noccs+d],-phi)*ss->data[ss_gld_i+l*Noccs+d]*ss_ret);
				gsl_matrix_set(Dco,Wl0_i+l,gld_i+l*Noccs+d,
						effic*pow(ss->data[ss_tld_i+l*Noccs+d],1.0-phi)*ss_ret);
	...
	/*
				// xld : d>0
				if(d+1==l){
					double pdd = effic*pow(ss->data[tld_i + l*Noccs+d],1.0-phi);
					gsl_matrix_set(Dvar,x_i +l*(Noccs+1)+d+1,x_i +l*(Noccs+1)+d+1,(1.0-sbar) + sbar*pdd*ss->data[gld_i+l*Noccs+d] );
					gsl_matrix_set(Dvar_tp1,x_i +l*(Noccs+1)+d+1,x_i +l*(Noccs+1)+d+1,1.0 );
					for(ll=0;ll<Noccs+1;ll++){
						if(ll!=d+1)
							gsl_matrix_set(Dvar,x_i +l*(Noccs+1)+d+1, x_i +ll*(Noccs+1)+d+1,tau*((1.0-sbar) + sbar*pdd*ss->data[gld_i+l*Noccs+d] ));
					}
					gsl_matrix_set(Dvar,x_i +l*(Noccs+1)+d+1,x_i +l*(Noccs+1),pdd*ss->data[gld_i+l*Noccs+d]);
				}
				else if(d+1 != l){
					double pld = effic*pow(ss->data[tld_i + l*Noccs+d],1.0-phi);
					gsl_matrix_set(Dvar,x_i +l*(Noccs+1)+d+1,x_i +l*(Noccs+1)+d+1,(1.0-sbar)*(1.0-tau) );
					gsl_matrix_set(Dvar_tp1,x_i +l*(Noccs+1)+d+1,x_i +l*(Noccs+1)+d+1,1.0 );

					if(l>0){
						gsl_matrix_set(Dvar,x_i +l*(Noccs+1)+d+1,x_i +l*(Noccs+1)+l,sbar*pld*ss->data[gld_i+l*Noccs+d] );
						for(ll=0;ll<Noccs+1;ll++){
							if(ll!=l)
							gsl_matrix_set(Dvar,x_i +l*(Noccs+1)+d+1,x_i +ll*(Noccs+1)+d+1,tau*sbar*pld*ss->data[gld_i+l*Noccs+d] );
						}
					}else{
						for(ll=0;ll<Noccs+1;ll++){
							for(dd=0;dd<Noccs;dd++){
								if(dd+1!=ll)
									gsl_matrix_set(Dvar,x_i +l*(Noccs+1)+d+1,x_i +ll*(Noccs+1)+dd+1,(1.0-tau)*sbar*pld*ss->data[gld_i+l*Noccs+d] );
							}
						}
					}
				}
				// xl0
				if(l>0)
					gsl_matrix_set(Dvar,x_i +l*(Noccs+1) ,x_i +(d+1)*(Noccs+1)+l,(1.0-findrtl)*sbar*tau );
				else{
					for(ll=0;ll<Noccs+1;ll++){
						if(ll!=d+1)
						gsl_matrix_set(Dvar,x_i +l*(Noccs+1) ,x_i +ll*(Noccs+1)+d+1,(1.0-findrtl)*(1.0-tau)*sbar );
					}
				}
	*/
			}
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			gdenom +=exp(sig_psi*effic*pow(ss->data[ss_tld_i+l*Noccs+d],1.0-phi)*
					(chi[l][d]*exp(Z+zd)-b+nud+beta*(ss->data[ss_Wld_i+l*Noccs+d]-ss->data[ss_Wl0_i+l])));
		}
		...
		for(d=0;d<Noccs;d++){
			...
			gsl_matrix_set(Dst_tp1,tld_i + l*Noccs+d,Wld_i + l*Noccs+d, //dt/dWld
				(beta*(1.0-phi)*effic/phi)*pow(tinside,1.0/phi-1.0)
				);

			gsl_matrix_set(Dst_tp1,tld_i + l*Noccs+d,Wl0_i + l, //dt/dWl0
				-1.0*(beta*(1.0-phi)*effic/phi)*pow(tinside,1.0/phi-1.0)
				);
			...
			double arr_d 	= (chi[l][d]*exp(Z+zd) -b+nud+beta*(ss->data[ss_Wld_i+l*Noccs+d]-ss->data[ss_Wl0_i+l]));
			...
			*/
			gsl_matrix_set(Dco,gld_i+l*Noccs+d,tld_i+l*Noccs+d,-1.0*
				(sig_psi*(1.0-phi)*effic*pow(ss->data[ss_tld_i+l*Noccs+d],-phi)*(arr_d)*gdenom
			...
			if(dd!=d){
				...
				double arr_dd 	= (chi[l][dd]*exp(Z+zd)-b+nudd+beta*(ss->data[ss_Wld_i+l*Noccs+dd]-ss->data[ss_Wl0_i+l]));
...
// derivatives for exog
	...
	for (l=0;l<Noccs+1;l++){
		...
		double dWl0dZ =0.0;
		for(d=0;d<Noccs;d++)
			dWl0dZ += effic*pow(ss->data[ss_tld_i+l*Noccs+d],1.0-phi)*chi[l][d];
		gsl_matrix_set(Dst,Wl0_i+l,0,dWl0dZ);
		for(d=0;d<Noccs;d++){
			double nud = l ==d+1? 0.0 : nu;
								// dWl0/dz
			gsl_matrix_set(Dst,Wl0_i+l,d+Notz,
					effic*pow(ss->data[ss_tld_i+l*Noccs+d],1.0-phi)*chi[l][d]);
			...
			double arr_d 	= (chi[l][d] -b+nud+beta*(ss->data[ss_Wld_i+l*Noccs+d]-ss->data[ss_Wl0_i+l]));
			...
			if(dd!=d){
				double nudd = l==dd+1 ? 0: nu;
				double arr_dd 	= (chi[l][dd]-b+nudd+beta*(ss->data[ss_Wld_i+l*Noccs+dd]-ss->data[ss_Wl0_i+l]));
...
int sim_moments(struct aux_coef* simdat,gsl_vector * ss,gsl_matrix * xss,struct sys_sol * sol,struct sys_coef * sys){
	...
	gsl_vector * Zz = gsl_vector_calloc(Nx);
	gsl_vector * fnd_l = gsl_vector_calloc(Noccs+1);
	gsl_vector * s_fnd_l = gsl_vector_calloc(Noccs+1);
	gsl_vector * s_x_u	= gsl_vector_calloc(Noccs+1);
	status = 0;
	...
	for(di = 0;di<Ndraw;di++){
		...
		if(urtp<0 || urtp>0.20){
			printf("Unemployment <0 or >.10");
		}
		// get the finding rate:

		if(printlev>=2){
			printmat("gld_T.csv", sol->gld);
			printmat("tld_T.csv", sol->tld);
		}
		double d_fnd =0.0;
		for(l=0;l<Noccs+1;l++){
			for(d=0;d<Noccs;d++){
				fnd_l->data[l] += gsl_matrix_get(sol->gld,l,d)*effic*pow(gsl_matrix_get(sol->tld,l,d),1.0-phi);
			}
			d_fnd += fnd_l->data[l]*gsl_matrix_get(xp,l,0)/urtp;
		}
		...
		for(l=0;l<Noccs+1;l++){
			for(d=0;d<Noccs;d++){
				d_wg += gsl_vector_get(x_u,l)*effic*pow(gsl_matrix_get(sol->tld,l,d),1.0-phi)*(chi[d][d] - chi[l][d]);
			}
		}
		...
		s_chng +=s_chng/(double)Ndraw;
		// sort fnd_l and put it in the average finding rate dist
		gsl_permutation * p = gsl_permutation_alloc(Noccs+1);
		gsl_sort_vector_index(p,fnd_l);
		gsl_permute_vector (p, fnd_l);
		gsl_permute_vector (p, x_u);
		gsl_vector_scale(fnd_l,(double)Ndraw);
		gsl_vector_add(s_fnd_l,fnd_l);

		gsl_vector_scale(x_u,(double)Ndraw);
		gsl_vector_add(s_x_u,x_u);
	...
	gsl_vector * dur_dist_ss = gsl_vector_alloc(5);
	gsl_vector * fnd_dist = gsl_vector_alloc(5);
	status += dur_dist(fnd_dist, dur_dist_ss,s_fnd_l,s_x_u);

	if(printlev>=1){
		printvec("fndrt_distrib_ss.csv",fnd_dist);
	}



	gsl_matrix_set(simdat->data_moments,0,0,avg_wg  - s_wg);
	gsl_matrix_set(simdat->data_moments,1,0,avg_urt - s_urt);
	gsl_matrix_set(simdat->data_moments,2,0,chng_pr - s_chng);

//	gsl_vector_free(fnd_dist);
//	gsl_vector_free(dur_dist_ss);

	gsl_vector_free(fnd_l);gsl_vector_free(Zzl);gsl_vector_free(shocks);
	gsl_matrix_free(gld);gsl_matrix_free(tld);
	gsl_matrix_free(x);gsl_matrix_free(xp);gsl_vector_free(Zz);gsl_vector_free(s_x_u);
	return status;
}
...
int dur_dist(gsl_vector * fnd_dist, gsl_vector * dur_dist, const gsl_vector * fnd_l, const gsl_vector * x_u){
	...
	for(l=0;l<Noccs+1;l++)
		gsl_vector_set(sort_x,l,gsl_vector_get(x_u,l));
	gsl_vector_memcpy(sort_fnd,fnd_l);
	...
	double cdf_x = 0.0;
	double dur0,dur1;
	double pctls[] = {0.25, 0.5, 0.75, 0.9, 1.0};
	int i=0;
	...
	for(l=0;l<Noccs+1;l++){
		...
		if(cdf_x>pctls[i]){
			dur_dist->data[i] = 0.5*dur0 + 0.5*dur1;;
			i++;
		}
		dur0 = dur1;
	}
	...
	gsl_matrix * Ft = gsl_matrix_alloc(Noccs+1,5); // 1 month, 3 months, 6 months, 1 yr, 2yrs
	double durs[] = {1.0,3.0,6.0,12.0,24.0};
	for(l=0;l<Noccs+1;l++){
		for(d =0;d<5;d++){
			double Ft_l = exp(-gsl_vector_get(fnd_l,l)*durs[d]);
			gsl_matrix_set(Ft,l,d,Ft_l );
		}
	}
	...
	for(d=0;d<5;d++){
		...
		double x_sur =0.0;
		for(l=0;l<Noccs+1;l++){
			x_sur += gsl_matrix_get(Ft,l,d)*gsl_vector_get(x_u,l);
			fnd_d += gsl_vector_get(fnd_l,l)*gsl_matrix_get(Ft,l,d)*gsl_vector_get(x_u,l);
		}
		gsl_vector_set(fnd_dist,d,fnd_d/x_sur);
	}
...
int gpol(gsl_matrix * gld, const gsl_vector * ss, const struct sys_coef * sys, const struct sys_sol * sol, const gsl_vector * Zz){
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			double nud = l == d+1? 0:nu;
			double zd = Zz->data[d+1+Ntfpl+Nfac*(Nflag)];
			gsl_matrix_set(gld, l,d,
					exp(sig_psi*gsl_matrix_get(pld,l,d)*
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			gsl_matrix_set(tld,l,d,
					mul*pow(chi[l][d]*exp(Z+zd) - b - nud + beta*(Es->data[Wld_i+l*Noccs+d] - Es->data[Wl0_i+l]),1.0/phi)
					);
		}
	}
...
//	b		= x[6];
	...
	status += sim_moments(simdat,ss,xss,sol,sys);
	int Nmo = (simdat->data_moments)->size1;
	dist = 0.0;
	for(i=0;i<Nmo;i++)
		dist += gsl_matrix_get(simdat->data_moments,i,0)*gsl_matrix_get(simdat->data_moments,i,0);
	if(verbose>=1){
		printf("At ef=%f,nu=%f,tau=%f\n",x[0],x[1],x[2]);
		printf("Calibration distance is %f\n",dist);
	}
	FILE * calhist;
	if(printlev>=1){
		calhist = fopen("calhist.txt","a");
		fprintf(calhist,"dist=%f, ef=%f,nu=%f,tau=%f\n",dist,x[0],x[1],x[2]);
		fclose(calhist);
	}
	gsl_matrix_free(xss);
	gsl_vector_free(ss);
