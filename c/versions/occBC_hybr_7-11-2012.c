...
int main(int argc,char *argv[]){
	...
	*/
	...
	double ub_0[]	= {0.75	,2.5		,0.03	,0.04		,0.20		,1.1,	.01		,.01	,.01};
...
int sim_moments(struct st_wr * st, gsl_vector * ss,gsl_matrix * xss){
	...
	gsl_matrix * Wt	= gsl_matrix_alloc(Noccs*(Noccs-1),Noccs*(Noccs-1));
			...
			for(l=1;l<Noccs+1;l++){
				for(d=0;d<Noccs;d++){
					if(l-1!=d){
						...
						if(gsl_finite(ylossld)){
							gsl_matrix_set(Wt,Xrow,Xrow,
								pld[l][d]*
								gsl_matrix_get(sol->gld,l,d)*x_u->data[l]/(1.0-x_u->data[0]));
