		...
		for(l=0;l<Noccs+1;l++){
			...
			for(d=0;d<Noccs;d++){
				...
				gsl_vector_set(ret_d,d,
					gsl_matrix_get(pld,l,d)*(chi[l][d] -nud - bl + post + beta*Wdif)
				);
			}
			...
			for(d=0;d<Noccs;d++){
				...
				double Wdif	= W_ld->data[l*Noccs+d] - W_l0->data[l];
				Wdif = Wdif<0.0 ?  0.0 : Wdif;
				double theta =	pow(effic*fm_shr/kappa,1.0/phi)*
						pow(chi[l][d] - nud -bl +beta*Wdif,1.0/phi) ;
				theta = theta<0? 1e-5 :  theta;
				theta = theta > pow(max_tss/effic,1.0/(1.0-phi)) ? pow(max_tss/effic,1.0/(1.0-phi)): theta;
				theta = theta > pow(max_tss/effic,-1.0/phi) ? pow(max_tss/effic,-1.0/phi): theta;
				/*if(thetald->data[l*Noccs+d] >-1000)
...
int sys_st_diff(gsl_vector * ss, gsl_matrix * Dst, gsl_matrix * Dco, gsl_matrix* Dst_tp1, gsl_vector * xx){
	...
	*/
			...
			for(d=0;d<Noccs;d++){
				...
				ss_ret += chi[l][d]*exp(Z+zd) -bl + post;
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			gsl_matrix_set(Dst_tp1,tld_i + l*Noccs+d,Wld_i + l*Noccs+d, //dt/dWld
				(beta*fm_shr*effic/phi/kappa)*pow(tinside,1.0/phi-1.0)
				);

			gsl_matrix_set(Dst_tp1,tld_i + l*Noccs+d,Wl0_i + l, //dt/dWl0
				-1.0*(beta*fm_shr*effic/phi/kappa)*pow(tinside,1.0/phi-1.0)
				);
