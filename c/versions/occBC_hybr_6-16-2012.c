...
int printlev 	= 3;
int use_anal	= 1;
int opt_alg		= 0; // 10x => NM, 5x => Subplex, o.w => BOBYQA
int polish_alg	= 0;
int homosk_zeta	= 1;
int gsl_fin_diffs=0;
...
double 	sig_psi	= 9.865434;//3.671249; 	// higher reduces the switching prob
...
double 	effic	= 1.474104;//1.162658;
double 	sbar	= 0.026949;	// will endogenize this a la Cheremukhin (2011)
// CHANGE THESE SOON SO THAT I CAN KEEP sbar right.
double 	scale_s	= 0.5;
double 	shape_s	= 0.10;
...
//double avg_sdsep= 	the average standard deviation of sld across occupations
double med_dr	= 13;
double chng_pr	= 0.45;
...
int main(){
	...
	for(l=1;l<Noccs+1;l++){
		for(d=0;d<Noccs;d++){
			...
			if(chi[l][d]>=chi_ub)
				child = chi_ub;
			chi[l][d] = (1.0 - child)*.9; // there is some selection, so the actual productivity is lower.
	...
	for(d=0;d<Noccs;d++){
		double mean_chi = 0.0;
		for(l=1;l<Noccs+1;l++)
			mean_chi = l != d+1 ? mean_chi+chi[l][d] : mean_chi;
		chi[0][d] = mean_chi/((double)Noccs-1.0 );
		gsl_matrix_set(chimat,0,d,chi[0][d]);
	}
	for(d=0;d<Noccs;d++)
		chi[d+1][d] = 1.0;
	if(printlev>=3) printmat("chimat.csv",chimat);
	...
	b[0] *= 0.5;
	//b[0] += 0.46; // 50% replacement ratio plus 0.46 flat for home production: Hall & Milgrom (2008)
	...
	if(homosk_zeta==1){
		...
		gsl_matrix_scale(var_zeta,sig_zet/144.0);
	}
	else
	...
	Nc = 2*Noccs*(Noccs+1);
	Nx = Noccs+1 + Nagf + Nfac*(Nllag+1);
	Notz 	= 1+ Nagf +Nfac*(Nllag+1);
...
*/
	...
	sig_psi = x[1];
	tau 	= x[2];
	double x0_0[]	= {phi,sig_psi,tau,sbar,effic,sk_wg[0],sk_wg[1],sk_wg[2],sk_wg[3]};
	double lb_0[]	= {0.15,0.005,0.001,0.015,0.1,sk_wg[0]*(0.7),sk_wg[1]*(0.7),sk_wg[2]*(0.7),sk_wg[3]*(0.7)}; // what to do about sig_psi?  Skipping that right now
	double ub_0[]	= {1.0,15.0,0.05,0.03,1.5,sk_wg[0]*(1.3),sk_wg[1]*(1.3),sk_wg[2]*(1.3),sk_wg[3]*(1.3)};
	...
	double x0_1[] = {x0_0[0],x0_0[1],x0_0[2],x0_0[3],x0_0[4]};
	double lb_1[] = {lb_0[0],lb_0[1],lb_0[2],lb_0[3],lb_0[4]};
	double ub_1[] = {ub_0[0],ub_0[1],ub_0[2],ub_0[3],ub_0[4]};
	int param_off = 5;
	st.cal_set = 1;
	st.n	= param_off;
	...
	objval = bndCalMinwrap(&cal_dist_df,lb_1,ub_1,param_off ,x0_1,&st,1e-7,1e-7,0.0,opt_alg,polish_alg);
	...
	// FIND CHI
	double x0_2[] = {x0_0[param_off],x0_0[param_off+1],x0_0[param_off+2],x0_0[param_off+3]};
	double lb_2[] = {lb_0[param_off],lb_0[param_off+1],lb_0[param_off+2],lb_0[param_off+3]};
	double ub_2[] = {ub_0[param_off],ub_0[param_off+1],ub_0[param_off+2],ub_0[param_off+3]};
	st.cal_set = 2;
	st.n	= 4;
	...
	//							f		lb ub n x0 param, ftol xtol, ctol , algo
	objval = bndCalMinwrap(&cal_dist,lb_2,ub_2,Nskill+1,x0_2,&st,1e-7,1e-7,0.0,opt_alg,polish_alg);
	if (verbose >= 1)printf("Final distance : %f from param set %d\n",objval, st.cal_set);
	fclose(calhist);
	fclose(solerr);
	fclose(simerr);
	...
	//							f		lb ub n x0 param, ftol xtol, ctol , algo
	objval = bndCalMinwrap(&cal_dist,lb_0,ub_0,param_off+Nskill+1,x0_0,&st,1e-7,1e-7,0.0,opt_alg,polish_alg);
	if (verbose >= 1)printf("Final distance : %f from param set %d\n",objval, st.cal_set);
 ...
 */
	...
	for(i = 0;i<maxrecur;i++){
		...
		if((fabs(mnorm)/mnorm0<1e-6) || (fabs(mnorml-mnorm)<1e-4 && fabs(mnorm)/mnorm0<1e-5))
			break;
...
int sol_ss(gsl_vector * ss, gsl_matrix * xss, struct sys_coef * sys){
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			gsl_matrix_set(sld,l*Noccs+d,sbar);
	...
	for(iter=0;iter<itermax;iter++){
		...
		for(l=0;l<Noccs+1;l++){
			...
			for(d=0;d<Noccs;d++){
				...
				// sld
				double cutoff = -(chi[l][d] - nud - bl + beta*Wdif);

				double sep_ld = scale_s*exp(shape_s*cutoff);
				if(cutoff<0.0)
					gsl_vector_set(sld,l*Noccs+d,sep_ld);
				else
					gsl_vector_set(sld,l*Noccs+d,scale_s); // should this be lower?  This means negative value for being here...
			}
			// adds to 1
			double gsum = 0.0;
...
//		gsl_matrix_set(N0,l+ Notz,0,0.0);
	...
	for(f=0;f<Nfac;f++){
		for(d=0;d<Noccs;d++)
			gsl_matrix_set(N0,d+Notz,f+1+Nagf,
					-gsl_matrix_get(LambdaCoef,d,f));
	}
	for(f=0;f<Nagf;f++)
		for(d=0;d<Noccs;d++)
			gsl_matrix_set(N0,d+Notz,f+1,
					-gsl_matrix_get(LambdaZCoef,d,f));
	 ...
	 * ( I     0 )
	 */
	set_matrix_block(N1,GammaCoef,1+Nagf,1+Nagf);
	for(f=0;f<Nfac*(Nllag);f++)
		gsl_matrix_set(N1,1+Nagf+Nfac+f,1+Nagf+f,1.0);
	/*
	...
	set_matrix_block(sys->S,var_eta,1+Nagf,1+Nagf);
	for(l=0;l<Noccs;l++){
	...
	for(l=0;l<Nfac*Nllag;l++)
		gsl_matrix_set(sys->S,1+Nagf+Nfac+l,1+Nagf+Nfac+l,0.0);
	...
	*/
			...
			for(d=0;d<Noccs;d++){
				...
				if(d+1 ==l){
					...
					gsl_matrix_set(Dst,Wld_i + l*Noccs+d, Wl0_i+l,-(1-tau)*ss->data[ss_sld_i + l*Noccs+d]);
				}
		...
		for(d=0;d<Noccs;d++){
			...
			if(l!=d+1){
				gsl_matrix_set(Dst,Wld_i + l*Noccs+d,0,
					// dWld/dZ
					(1.0- ss->data[ss_sld_i + l*Noccs+d])*(1.0-tau)*chi[l][d]+(1.0-ss->data[ss_sld_i + (d+1)*Noccs+d])*tau*chi[d+1][d] );
				gsl_matrix_set(Dst,Wld_i + l*Noccs+d,d+Notz,
					// dWld/dzd
					((1.0-ss->data[ss_sld_i + l*Noccs+d])*(1.0-tau)*chi[l][d]+
							(1.0-ss->data[ss_sld_i + (d+1)*Noccs+d])*tau*chi[d+1][d]) );
			}
			else{
				gsl_matrix_set(Dst,Wld_i + l*Noccs+d,0,
						// dWld/dZ
						(1.0-ss->data[ss_sld_i + l*Noccs+d])*chi[l][d]);
				gsl_matrix_set(Dst,Wld_i + l*Noccs+d,d+Notz,
						// dWld/dzd
						(1.0-ss->data[ss_sld_i + l*Noccs+d])*chi[l][d]);
			}
...
int sim_moments(struct aux_coef * simdat,gsl_vector * ss,gsl_matrix * xss,struct sys_sol * sol,struct sys_coef * sys){
	...
	double s_fnd,s_urt,s_wg,s_chng,m_Zz, s_cvu,s_sdu,s_sdZz,s_elpt;
	...
	for(di=0;di<init_T;di++){
		...
		status += theta(sol->sld, ss, sys, sol, Zz);
...
int gpol(gsl_matrix * gld, const gsl_vector * ss, const struct sys_coef * sys, const struct sys_sol * sol, const gsl_vector * Zz){
	...
	int ss_x_i,ss_gld_i, ss_Wld_i,ss_Wl0_i;
		ss_x_i		= 0;
		ss_Wl0_i	= 0;//ss_x_i + pow(Noccs+1,2);
		ss_Wld_i	= ss_Wl0_i + Noccs+1;
...
int spol(gsl_matrix * sld, const gsl_vector * ss, const struct sys_coef * sys, const struct sys_sol * sol, const gsl_vector * Zz){
	...
	int Wl0_i = 0;
		int Wld_i = Wl0_i + Noccs+1;
		int ss_x_i,ss_gld_i, ss_tld_i, ss_Wld_i,ss_Wl0_i;
			ss_x_i		= 0;
			ss_Wl0_i	= 0;//ss_x_i + pow(Noccs+1,2);
			ss_Wld_i	= ss_Wl0_i + Noccs+1;
			ss_gld_i	= ss_Wld_i + Noccs*(Noccs+1);//x_i + pow(Noccs+1,2);
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			double cutoff= - (chi[l][d]*exp(Z+zd) - bl - nud + beta*cont );
			double sep_ld = scale_s*exp(shape_s*cutoff);
			if(cutoff<0.0)
				gsl_matrix_set(sld,l,d,sepld);
			else
...
int xprime(gsl_matrix * xp, gsl_vector * ss, const struct sys_coef * sys, const struct sys_sol * sol, const gsl_matrix * x, const gsl_vector * Zz){
	...
	for(k=0;k<Noccs+1;k++){
		for(j=1;j<Noccs+1;j++){
			if(j!=k)
				newdisp += gsl_matrix_get(sld,k,j)*(1.0-tau)*gsl_matrix_get(x,k,j);
		}
	}
	...
	for(l=1;l<Noccs+1;l++){
		for(d=0;d<Noccs;d++){
			ald[l][d] = gsl_matrix_get(sol->gld,l,d)*(gsl_matrix_get(x,l,0) + gsl_matrix_get(sol->sld,l,l-1)*gsl_matrix_get(x,l,l));
			for(k=0;k<Noccs+1;k++){
				if(k!=l)
					ald[l][d]+= gsl_matrix_get(sol->gld,l,d)*tau*gsl_matrix_get(sol->sld,k,l)*gsl_matrix_get(x,k,l);
			}
		}
	}
	for(l=0;l<Noccs+1;l++){
	...
	for(l=1;l<Noccs+1;l++){
		...
		gsl_matrix_set(xp,l,0,
			(1.0-findrt[l])*(gsl_matrix_get(x,l,0) + gsl_matrix_get(sol->sld,l,l-1)*gsl_matrix_get(x,l,l) + tau*gsl_matrix_get(sol->sld,l,l-1)**sxpjl)
			);
	}
		...
		for(d=0;d<Noccs;d++){
			if(l!=d+1)
				gsl_matrix_set(xp,l,d+1,
					(1.0-tau)*(1.0-gsl_matrix_get(sol->sld,l,d))*gsl_matrix_get(x,l,d+1) + pld[l][d]*ald[l][d]
					);
			else{
				double newexp = 0.0;
				for(j=0;j<Noccs+1;j++)
				...
				gsl_matrix_set(xp,l,d+1,
					(1.0-gsl_matrix_get(sol->sld,l,d))*(gsl_matrix_get(x,l,d+1)+newexp )+ pld[l][d]*ald[l][d]
					);
			}
		}
	}
...
int set_params(const double * x, int cal_set){
	...
	//do something about chi below b
...
double cal_dist(unsigned n, const double *x, double *grad, void* params){
	...
	if(st->cal_set == 0 || st->cal_set ==1){
		...
		if(verbose>=1){
			printf("At phi=%f,sig_psi=%f,tau=%f,sbar=%f,ef=%f\n",x[0],x[1],x[2],x[3],x[4]);
