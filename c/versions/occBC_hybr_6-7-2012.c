...
double 	sig_psi	=1; 	// higher reduces the switching prob

double *	b; // unemployment benefit


double ** chi;
double 	tau 	= 0.0099;//0.003857;
double 	kappa	= 0.45;//.1306; corresponds to HM number
double 	phi		= 0.6;//0.4;	// : log(1/UE elasticity -1) = phi*log(theta)= log(1/.38-1)/log(5)
double 	fm_shr	= 0.8;	// firm's surplus share
double 	effic	= 1.0;
double 	sbar	= 0.0195;	// will endogenize this a la Cheremukhin (2011)
...
double avg_elpt	= 0.38;   // elasticity of p wrt theta
double med_dr	= 13;
double chng_pr	= 0.45;
...
*/
	...
	int param_off = 5;
	st.cal_set = 1;
	FILE * calhist = fopen("calhist1.txt","w+");
	...
	fclose(simerr);
	//							f		lb ub n x0 param, ftol xtol, ctol , algo
	//objval = bndCalMinwrap(&cal_dist,lb_0,ub_0,8,x0_0,&st,1e-7,1e-7,0.0,opt_alg);
	//if (verbose >= 1)printf("Final distance : %f from param set %d\n",objval, st.cal_set);
 ...
 */
	...
	if(printlev>=2){
		FILE * errlog = fopen("solerr.log","a");
		fprintf(errlog,"Non-finite %d times in P0-P2\n",nonfin);
		fclose(errlog);
	}
	...
	if(i>=maxrecur-1){
		FILE * errlog = fopen("solerr.log","a");
		fprintf(errlog,"Forward expectations stopped at %f, did not converge\n",mnorm);
		fclose(errlog);
	}
...
*/
		...
		#pragma omp parallel for default(shared) private(d,l)
		for(l=0;l<Noccs+1;l++){
			double bl = l>0 ? b[1]:b[0];
			for(d=0;d<Noccs;d++){
...
//		gsl_matrix_set(N0,l+ Notz,0,0.0);
	...
	if(printlev>=1){
		FILE * errlog = fopen("solerr.log","a");
		fprintf(errlog,"Defining the system had %d \n",nonfin);
		fclose(errlog);
	}
...
int sim_moments(struct aux_coef * simdat,gsl_vector * ss,gsl_matrix * xss,struct sys_sol * sol,struct sys_coef * sys){
	...
	FILE * errlog = fopen("simerr.log","a");
...
double cal_dist(unsigned n, const double *x, double *grad, void* params){
	...
	if(status == 0)
		...
		gsl_matrix_scale(simdat->data_moments,1.5);
		FILE * errlog = fopen("solerr.log","a");
		fprintf(errlog,"System did not solve at: (%f,%f,%f,%f)\n",x[0],x[1],x[2],x[3]);
		fclose(errlog);
	}	
