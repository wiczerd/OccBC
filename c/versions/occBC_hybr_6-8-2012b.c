...
double 	sig_psi	=2.313858;//1; 	// higher reduces the switching prob
...
double 	phi		= 0.6;	// : log(1/UE elasticity -1) = phi*log(theta)= log(1/.38-1)/log(5)
double 	fm_shr	= 0.9;	// firm's surplus share
double 	effic	= 0.995;
double 	sbar	= 0.023705;	// will endogenize this a la Cheremukhin (2011)
...
*/
	...
	double lb_0[]	= {0.15,0.005,0.001,0.015,0.1,x0_0[3]*(0.7),x0_0[4]*(0.7),x0_0[5]*(0.7),x0_0[6]*(0.7)}; // what to do about sig_psi?  Skipping that right now
	double ub_0[]	= {1.0,15.0,0.05,0.03,1.5,x0_0[3]*(1.3),x0_0[4]*(1.3),x0_0[5]*(1.3),x0_0[6]*(1.3)};
	...
	//							f		lb ub n x0 param, ftol xtol, ctol , algo
	objval = bndCalMinwrap(&cal_dist,lb_2,ub_2,4,x0_2,&st,1e-7,1e-7,0.0,opt_alg,polish_alg);
	//if (verbose >= 1)printf("Final distance : %f from param set %d\n",objval, st.cal_set);
	fclose(calhist);
	fclose(solerr);
	fclose(simerr);
	...
	//							f		lb ub n x0 param, ftol xtol, ctol , algo
	objval = bndCalMinwrap(&cal_dist,lb_0,ub_0,8,x0_0,&st,1e-7,1e-7,0.0,opt_alg,polish_alg);
	//if (verbose >= 1)printf("Final distance : %f from param set %d\n",objval, st.cal_set);
