...
double 	sig_psi	=5.627577; // higher reduces the switching prob
...
double 	kappa	= .1338;
double 	phi		= 0.45;	// match elasticity
double 	fm_shr	= 0.8;	// firm's surplus share
double 	effic	= 0.75;
double 	sbar	= 0.0262;	// will endogenize this a la Cheremukhin (2011)
...
/*	simdat.data_moments->data[0] = avg_sep;
	...
	simdat.moment_weights->data[1] = 20.0;
	...
	double lb[]	= {0.005,0.0,0.0001,x0[3]*(0.7),x0[4]*(0.7),x0[5]*(0.7),x0[6]*(0.7)}; // what to do about sig_psi?  Skipping that right now
	double ub[]	= {1.0,1.0,0.05,x0[3]*(1.3),x0[4]*(1.3),x0[5]*(1.3),x0[6]*(1.3)};
	for(i=0;i<7;i++){
		if (lb[i]>=ub[i]){
			double lbi = lb[i];
			lb[i] = ub[i];
