...
*  rev: 5-06-2012
*
* compile line: icc occBC_hybr.c -static -lgsl -openmp -mkl=parallel -lnlopt -I/home/david/Computation -I/home/david/Computation/gsl/gsl-1.15 -o occBC_hybr.out
*/
...
double 	sig_psi	=.124; 	// higher reduces the switching prob
...
*/
	...
	gsl_matrix_set(simdat.data_moments,0,3,avg_elpt);
	...
	gsl_matrix_set(simdat.moment_weights,0,3,1.0);
	...
	double lb_0[]	= {0.15,0.05,0.001,0.015,0.1,x0_0[3]*(0.7),x0_0[4]*(0.7),x0_0[5]*(0.7),x0_0[6]*(0.7)}; // what to do about sig_psi?  Skipping that right now
	...
	double x0_1[] = {x0_0[0],x0_0[1],x0_0[2],x0_0[3]};
	double lb_1[] = {lb_0[0],lb_0[1],lb_0[2],lb_0[3]};
	double ub_1[] = {ub_0[0],ub_0[1],ub_0[2],ub_0[3]};
	int param_off = 4;
	st.cal_set = 1;
	FILE * calhist = fopen("calhist.txt","w+");
	...
	FILE * solerr = fopen("solerr.log","w+");
	fprintf(solerr,"Errors while solving model with param set 1\n");
	fclose(solerr);
	FILE * simerr = fopen("simerr.log","w+");
...
//		gsl_matrix_set(simdat->data_moments,0,3,(avg_elpt - s_elpt)/avg_elpt);
	}
...
double cal_dist(unsigned n, const double *x, double *grad, void* params){
	...
	if(st->cal_set == 0 || st->cal_set ==1){
		...
		if(verbose>=1){
			printf("At phi=%f,sig_psi=%f,tau=%f,ef=%f\n",x[0],x[1],x[2],x[3]);
