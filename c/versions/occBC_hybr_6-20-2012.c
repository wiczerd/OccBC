...
*/
	...
	/* Calibration Loop!
	*/
	double x0_0[]	= {phi,sig_psi,tau,scale_s,shape_s,effic,sk_wg[0],sk_wg[1],sk_wg[2],sk_wg[3]};
	double lb_0[]	= {0.15,0.005,0.001,0.015,0.001,0.1,sk_wg[0]*(0.7),sk_wg[1]*(0.7),sk_wg[2]*(0.7),sk_wg[3]*(0.7)}; // what to do about sig_psi?  Skipping that right now
	double ub_0[]	= {1.0,15.0,0.05,0.03, 0.5,1.5,sk_wg[0]*(1.3),sk_wg[1]*(1.3),sk_wg[2]*(1.3),sk_wg[3]*(1.3)};
	...
	objval = bndCalMinwrap(&cal_dist_df,(lb_1.vector).data,(ub_1.vector).data,param_off ,(x0_1.vector).data,&st,1e-7,1e-7,0.0,opt_alg,polish_alg);
...
int sys_st_diff(gsl_vector * ss, gsl_matrix * Dst, gsl_matrix * Dco, gsl_matrix* Dst_tp1, gsl_vector * xx){
	int l,d,status;
	int tld_i,sld_i,x_i,gld_i,Wld_i,Wl0_i;
	x_i 	= 0;
	Wl0_i	= 0;
	Wld_i	= Wl0_i + Noccs+1;
	// for Dco:
	...
	*/
			...
			for(d=0;d<Noccs;d++){
				...
				if(d+1 ==l){
					...
					gsl_matrix_set(Dco,Wld_i + l*Noccs+d, sld_i+l*Noccs+d,
						-(1.0-tau)*(cont_ld - ss->data[ss_Wl0_i+0]));
				}
...
int spol(gsl_matrix * sld, const gsl_vector * ss, const struct sys_coef * sys, const struct sys_sol * sol, const gsl_vector * Zz){
	...
	int Wl0_i = 0;
		int Wld_i = Wl0_i + Noccs+1;
		int ss_x_i,ss_gld_i, ss_tld_i, ss_sld_i,ss_Wld_i,ss_Wl0_i;
			ss_x_i		= 0;
			ss_Wl0_i	= 0;//ss_x_i + pow(Noccs+1,2);
			ss_Wld_i	= ss_Wl0_i + Noccs+1;
			ss_gld_i	= ss_Wld_i + Noccs*(Noccs+1);//x_i + pow(Noccs+1,2);
	...
	#pragma parallel for private(l,d)
	for(l=0;l<Noccs+1;l++){
		double bl = l>0 ? b[1]:b[0];
		double W0 = 0.0;
		for(d=0;d<Noccs;d++){
