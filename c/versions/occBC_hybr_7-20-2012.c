...
double 	fm_shr	= 0.66;	// firm's surplus share
...
//phi=0.419535,sig_psi=1.499964,tau=0.019036,scale_s=0.032432,shape_s=0.099990,effic=1.095033
//obj = .64 <=  0.442140,0.507194,0.009450,0.034141,0.063135,1.099987
// 0.459286,2.451550,0.056732,0.032158,0.082126,1.073938
// 0.406220,1.417713,0.052547,0.033242,0.102571,1.385739,

double ** chi;

//0.598887,1.401607,0.032296,0.042518,0.097938,0.684575,
//0.599054,1.399568,0.032652,0.044994,0.105415,0.671545,
// 0.521993,1.120764,0.034911,0.036268,0.126064,0.750001,-3.809410,-3.974563,-2.417442,

double 	phi		= 0.521993;	// : log(1/UE elasticity -1) = phi*log(theta)= log(1/.38-1)/log(5)
double 	sig_psi	= 1.120764; 	// higher reduces the switching prob
double 	tau 	= 0.034911;
double 	scale_s	= 0.036268;
double 	shape_s	= 0.126064;
double 	effic	= 0.750001;


double rhoZ		= 0.9732;
double rhozz	= 0.99;

double sig_eps	= 0.00008557;//0.00002557;//7.2445e-7;  (using the FRED Number, rather than mine)
...
int sol_ss(gsl_vector * ss, gsl_matrix * xss, struct sys_sol *sol);
...
int main(int argc,char *argv[]){
	...
	double chi_coef0[] ={-3.809410,-3.974563,-2.417442};//{-3.492175,-1.987377,-1.242499};
	...
	if(homosk_zeta==1){
		...
		gsl_matrix_scale(var_zeta,pow(3.1144,2)); // adjust so that has the same unconditional variance as with the true persistence
	}
	else
		readmat("var_zeta.csv",var_zeta);
	...
	/* Calibration Loop!
	*/
	double x0_0[]	= {phi	,sig_psi	,tau	,scale_s	,shape_s	,effic,	chi_coef0[0]	,chi_coef0[1]	,chi_coef0[2]};
	double lb_0[]	= {0.35	,0.001		,0.005	,0.015		,0.005		,0.5	,-5.0			,-5.0			,-5.0	}; // what to do about sig_psi?  Skipping that right now
	double ub_0[]	= {0.75	,3.5		,0.06	,0.05		,0.20		,1.5	,.01				,.01		,.01	};

	for(i=0;i<6+Nskill;i++){
		if (lb_0[i]>=ub_0[i]){
			double lbi = lb_0[i];
			lb_0[i] = ub_0[i];
			ub_0[i] = lbi;
	...
	gsl_vector_view x0vw = gsl_vector_view_array (x0_0, 9);
	gsl_vector_view lbvw = gsl_vector_view_array (lb_0, 9);
	gsl_vector_view ubvw = gsl_vector_view_array (ub_0, 9);
...
//			objval = bndCalMinwrap(&quad_solve,(lb_1.vector).data,(ub_1.vector).data,param_off ,(x0_1.vector).data,st,1e-7,1e-7,0.0,opt_alg,0);
			objval = bndCalMinwrap(&cal_dist,(lb_1.vector).data,(ub_1.vector).data,param_off ,(x0_1.vector).data,st,1e-7,1e-7,0.0,opt_alg,polish_alg);
		...
		if(calflag == 2 || calflag==0){
			...
			//							f		lb ub n x0 param, ftol xtol, ctol , algo
			objval = bndCalMinwrap(&cal_dist,(lb_2.vector).data,(ub_2.vector).data,Nskill,(x0_2.vector).data,st,1e-7,1e-7,0.0,opt_alg,polish_alg);
			if (verbose >= 1)printf("Final distance : %f from param set %d\n",objval, st->cal_set);
	...
	status += sol_ss(st->ss,st->xss,st->sol);
...
//	if(verbose>=0 && status ==0) printf("Shocks successfully calibrated\n");
	...
	double urt,opt_urt;
	if(status ==0)
		status +=  TGR(u_dur_dist,opt_dur_dist,fnd_dist,opt_fnd_dist,&urt,&opt_urt,st->ss,st->sol,st->sys);
...
int sol_ss(gsl_vector * ss, gsl_matrix * xss, struct sys_sol * sol){
	...
	for(iter=0;iter<itermax;iter++){
		// I will solve this by a form of Howard's acceleration: iterate on VFs a few times before going to the policy functions
		int vfiter;
			...
			for(l=0;l<Noccs+1;l++){
				...
				if(l-1!=d){
					gsl_vector_set(W_ld,l*Noccs+d,
						((1.0-tau)*((1.0-sepld )*chi[l][d] + sepld*W_l0->data[0] ) + tau*W_ld->data[(d+1)*Noccs+d]  )
...
*/
		...
		for(l=0;l<Noccs+1;l++){
			...
			for(d=0;d<Noccs;d++){
				...
				double Wdif	= W_ld->data[l*Noccs+d] - W_l0->data[l];
				//Wdif = Wdif<0.0 ? 0.0 : Wdif;
				double surp = chi[l][d] - nud - bl +beta*Wdif;
				if(surp > 0.0){
		...
		if(break_flag ==1)
			...
			break_flag=1;
		else if((fabs(maxdistg - lmaxdistg) < ss_tol*3) & (iter>100)){
			break_flag=1;
	...
	//	double wld_ld = (1.0-fm_shr)*chi[l][d]- fm_shr*beta*(bl-Exi);
	//	double wld_ld = ((1.0-fm_shr)*chi[l][d]- fm_shr*beta*(W_ld->data[l*Noccs+d] - W_l0->data[l] + bl-Exi)
	//			+ beta*(1.0-sld->data[l*Noccs+d])*chi[l][d]/(1.0-beta*(1.0-sld->data[l*Noccs+d])) )
	//					/(1.0 + beta/(1.0-beta*(1.0-sld->data[l*Noccs+d])));
		double wld_ld = (1.0-fm_shr)*chi[l][d] + fm_shr*(Exi*(1.0-beta*(1.0-sld->data[l*Noccs+d]))
		...
		wld_ld = wld_ld > chi[l][d] ? chi[l][d] : wld_ld;
		wld_ld = chi[l][d];
		gsl_matrix_set(sol->ss_wld,l,d,wld_ld);
		Jd[d] = (1.0-sld->data[l*Noccs+d])*(chi[l][d] - wld_ld)/(1.0-(1.0-sld->data[l*Noccs+d])*beta);
	}
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			if(d!=l-1){
				...
				double Ephi = 0.0;
//				double wld_ld = (1.0-fm_shr)*chi[l][d]- fm_shr*beta*(bl - Ephi) ;

//				double wld_ld = ((1.0-fm_shr)*chi[l][d]- fm_shr*beta*(W_ld->data[l*Noccs+d] - W_l0->data[l] + bl - Ephi)
//						+ beta*((1.0-sld->data[l*Noccs+d])*(1.0-tau)*chi[l][d] +tau*Jd[d] )/(1.0-beta*(1.0-sld->data[l*Noccs+d])*(1.0-tau) ))
//								/(1.0+ beta/(1.0-beta*(1.0-tau)*(1.0-sld->data[l*Noccs+d])));
				double wld_ld = (1.0-fm_shr)*chi[l][d] + fm_shr*(
						bl + beta*W_l0->data[l] - beta*W_ld->data[l*Noccs+d])
			...
			for(d=0;d<Noccs;d++){
				...
				if(ss->data[ss_tld_i+l*Noccs+d]>0){
					if(d+1 ==l){
						...
						gsl_matrix_set(Dst,Wld_i + l*Noccs+d, Wl0_i+0,-(1-tau)*ss->data[ss_sld_i + l*Noccs+d]
								*ss->data[ss_Wl0_i+l]/Wld_ss
								);
				...
				double dtld = ss->data[ss_gld_i+l*Noccs+d]*(
								ss_ret*dpmatch(ss->data[ss_tld_i+l*Noccs+d])
				...
				if(pld>0.0 && gsl_finite(pld*ss_ret))
					gsl_matrix_set(Dco,Wl0_i+l,gld_i+l*Noccs+d,pld*ss_ret
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			if(tld_ss>0.0){
				...
				//double dtld_old =beta*pow(tld_ss*kappa/(fm_shr*effic*surp),1.0-phi)*(fm_shr*effic)/kappa;
			...
			if(dd!=d){
				if(ss->data[ss_gld_i+l*Noccs+d]>0.0 && ss->data[ss_tld_i+l*Noccs+dd]>0.0){
					...
					double dgdWld	= -pldd*beta*sig_psi
							*ss->data[ss_gld_i+l*Noccs+dd]*ss->data[ss_gld_i+l*Noccs+d];
					if(arr_dd>0.0 && ss->data[ss_tld_i+l*Noccs+dd]>0.0)
...
//	int Notz = 1 + Nagf + Nfac*(Nflag+1);
	...
	for (l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			double post	= pld>0 ? - kappa*ss->data[ss_tld_i+l*Noccs+d]/pld : 0.0;
								// dWl0/dz
			double dWdz =ss->data[ss_gld_i+l*Noccs+d]*chi[l][d]*pld;
			...
			if(ss->data[ss_tld_i+l*Noccs+d]>0.0){
				...
				double dtdz =dqdz*dinvq(kappa/(fm_shr*surp));//*pow(ss->data[ss_tld_i+l*Noccs+d]*kappa/(fm_shr*effic*surp),1.0-phi)*fm_shr*effic/kappa;
...
int sim_moments(struct st_wr * st, gsl_vector * ss,gsl_matrix * xss){
	...
	gsl_matrix * urt_l, * urt_l_wt, * fnd_l_hist, * x_u_hist, * Zzl_hist, * s_wld,*tld_hist;
	if(printlev>=2){
		urt_l		= gsl_matrix_calloc(Ndraw,Noccs);
		fnd_l_hist	= gsl_matrix_calloc(Ndraw,Noccs+1);
		tld_hist	= gsl_matrix_calloc(Ndraw,Noccs);
	...
	for(di = 0;di<Ndraw;di++){
		...
		if(printlev>=2){
			for(d=1;d<Noccs+1;d++){
				...
				gsl_matrix_set(tld_hist,di,d-1,0.0);
				tld_hist->data[di*tld_hist->tda+d-1] = gsl_matrix_get(sol->tld,d,d-1);
				/*double xd = 0.0;
				for(l=0;l<Noccs+1;l++){
					tld_hist->data[di*tld_hist->tda+d-1] += gsl_matrix_get(x,l,d)*gsl_matrix_get(sol->tld,l,d-1);
					xd += gsl_matrix_get(x,l,d);
				}
				tld_hist->data[di*tld_hist->tda+d-1] /= xd;
				*/
			}
		}
		...
		for(l=0;l<Noccs+1;l++){
			for(d=0;d<Noccs;d++)
				m_tld += gsl_matrix_get(x,l,d+1)*sol->tld->data[l*Noccs+d];
		}
	...
	if(verbose>=3){
		...
		printf("urt=%f, ",gsl_matrix_get(simdat->data_moments,0,3));
		printf("elpt=%f, ",gsl_matrix_get(simdat->data_moments,0,4));
		printf("sdsep=%f, ",gsl_matrix_get(simdat->data_moments,0,5));
	...
	for(t=0;t<Ndraw;t++){
		...
		for(l=0;l<Noccs;l++){
			if(gsl_matrix_get(fnd_l_hist,t,l)>1e-8)
				d_dur += 1.0/gsl_matrix_get(fnd_l_hist,t,l)*gsl_matrix_get(x_u_hist,t,l);
		}
