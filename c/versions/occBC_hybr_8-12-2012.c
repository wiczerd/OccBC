...
/*		for(l=0;l<Noccs+1;l++){
			for(d=0;d<Noccs;d++){
				if(gsl_matrix_get(pld_hist,t,l*Noccs+d)>1e-4)
					d_dur += 1.0/gsl_matrix_get(pld_hist,t,l*Noccs+d)*gsl_matrix_get(x_u_hist,t,l)*gsl_matrix_get(gld_hist,t,l*Noccs+d);
			}
		}
*/
		double d_xdur = 0.0;
		for(duri=0;duri<4;duri++){
			double fdur=0.0;
			double xdur=0.0;
			for(l=0;l<Noccs+1;l++){
				for(d=0;d<Noccs;d++){
					fdur = gsl_matrix_get(x_u_hist,t,l)*gsl_matrix_get(gld_hist,t,l*Noccs+d)
							*exp(- gsl_matrix_get(pld_hist,t,l*Noccs+d)*durs[duri])*gsl_matrix_get(pld_hist,t,l*Noccs+d);
					xdur = gsl_matrix_get(x_u_hist,t,l)*gsl_matrix_get(gld_hist,t,l*Noccs+d)
							*exp(- gsl_matrix_get(pld_hist,t,l*Noccs+d)*durs[duri]);
				}
			}
			d_dur += 1.0/(fdur/xdur)*xdur;
			d_xdur += xdur;
		}
		d_dur /= d_xdur;
		...
		for(l=0;l<Noccs+1;l++){
			for(d=0;d<Noccs;d++){
				if(gsl_matrix_get(pld_hist,t,l*Noccs+d)>1e-4)
					d_sddur += pow(gsl_matrix_get(pld_hist,t,l*Noccs+d),-2)*gsl_matrix_get(gld_hist,t,l*Noccs+d)*gsl_matrix_get(x_u_hist,t,l);
			}
		}
