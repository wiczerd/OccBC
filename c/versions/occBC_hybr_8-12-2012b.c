...
int main(int argc,char *argv[]){
	...
	*/
	...
	double lb_0[]	= {0.35	,0.0005		,0.005	,0.02		,0.005		,0.35	,-2.5			,-2.5			,-2.5	}; // what to do about sig_psi?  Skipping that right now
	double ub_0[]	= {0.8	,2.5		,0.1	,0.05		,0.20		,1.25	,.01			,.01			,.01	};
...
int dur_dist(struct dur_moments * s_mom, const gsl_matrix * gld_hist, const gsl_matrix * pld_hist, const gsl_matrix * x_u_hist){
	...
	for(duri =0;duri <5;duri ++){
		...
		for(t=0;t<Ndraw;t++){
			for(l=0;l<Noccs+1;l++){
				for(d=0;d<Noccs;d++){
					Ft_l += gsl_matrix_get(x_u_hist,t,l)*gsl_matrix_get(gld_hist,t,l*Noccs+d)
							*exp(- gsl_matrix_get(pld_hist,t,l*Noccs+d)*durs[duri])*gsl_matrix_get(pld_hist,t,l*Noccs+d);
					xt_l += gsl_matrix_get(x_u_hist,t,l)*gsl_matrix_get(gld_hist,t,l*Noccs+d)
							*exp(- gsl_matrix_get(pld_hist,t,l*Noccs+d)*durs[duri]);
				}
			}
		}
		Ft_l /= xt_l;
		gsl_vector_set(s_mom->Ft,duri,Ft_l);
	}
	...
	for(t=0;t<Ndraw;t++){
		if(t<Ndraw-1){
			for(l=0;l<Noccs+1;l++){
				fnd_l->data[l] =0.0;
					for(d=0;d<Noccs;d++)
						fnd_l->data[l] += gsl_matrix_get(gld_hist,t+1,l*Noccs+d)*gsl_matrix_get(pld_hist,t+1,l*Noccs+d);
			}
		}
		double d_dur = 0.0;
		for(l=0;l<Noccs+1;l++){
			for(d=0;d<Noccs;d++){
				d_dur += ((1.0 + 1.0/fnd_l->data[l])*(1.0-gsl_matrix_get(pld_hist,t,l*Noccs+d))
						+gsl_matrix_get(pld_hist,t,l*Noccs+d)
						)*gsl_matrix_get(x_u_hist,t,l)*gsl_matrix_get(gld_hist,t,l*Noccs+d);
			}
		}
		...
		for(l=0;l<Noccs+1;l++){
			for(d=0;d<Noccs;d++){
				if(gsl_matrix_get(pld_hist,t,l*Noccs+d)>1e-4)
								// variance of the first period
					d_sddur += (pow(gsl_matrix_get(pld_hist,t,l*Noccs+d),2)*(1.0-gsl_matrix_get(pld_hist,t,l*Noccs+d))
							+ (1.0-gsl_matrix_get(pld_hist,t,l*Noccs+d))*pow(fnd_l->data[l],-2))
							*gsl_matrix_get(gld_hist,t,l*Noccs+d)*gsl_matrix_get(x_u_hist,t,l);
			}
		}
		...
		for(l=0;l<Noccs;l++){
			for(d=0;d<Noccs;d++)
				d_6mo += pow(1 - gsl_matrix_get(pld_hist,t,l*Noccs+d),6)*gsl_matrix_get(gld_hist,t,l*Noccs+d)*gsl_matrix_get(x_u_hist,t,l);
		}
