	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			double gld_ld 	=exp(sig_psi*gsl_matrix_get(pld,l,d)*
					(chi[l][d]*exp(Z + zd) - b - nud +post + beta*(Es->data[Wld_i + l*Noccs+d] -Es->data[Wl0_i + l])) );
		...
		for(d=0;d<Noccs;d++){
			...
			double post = -kappa/effic*pow(gsl_matrix_get(sol->tld,l,d),phi);
			double gld_ld =exp(sig_psi*gsl_matrix_get(pld,l,d)*
					(chi[l][d]*exp(Z + zd) - b - nud + post + beta*(Es->data[Wld_i + l*Noccs+d] -Es->data[Wl0_i + l])) )/gdenom;
