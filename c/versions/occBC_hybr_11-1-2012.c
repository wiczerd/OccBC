...
int sol_ss(gsl_vector * ss, gsl_vector * Zz, gsl_matrix * xss, struct sys_sol * sol){
	...
	gsl_vector * v_distg = gsl_vector_calloc(itermax);
	int break_flag =0;
	double Z = Zz->data[0];
	...
	for(iter=0;iter<itermax;iter++){
		...
		gsl_vector_set(v_distg,iter,maxdistg);
...
int xprime(gsl_matrix * xp, gsl_vector * ss, const struct sys_coef * sys, const struct sys_sol * sol, const gsl_matrix * x, const gsl_vector * Zz){
		...
		for(ll=0;ll<2;ll++){
			for(d=0;d<Noccs;d++)
				ald[ll*(Noccs+1)+l][d] = gsl_matrix_get(gld,l+ll*(Noccs+1),d)*(gsl_matrix_get(x,l,ll*(Noccs+1)) +
						(1.0-(double)ll)*gsl_matrix_get(sld,l,l-1)*(gsl_matrix_get(x,l,l)+tau*sxpjl) );
		}
	}
	for(l=0;l<Nl;l++){
		for(d=0;d<Noccs;d++)
	...
	for(l=1;l<Noccs+1;l++){
		...
		gsl_matrix_set(xp,l,0,
				(1.0-findrt[l])*(gsl_matrix_get(x,l,0)*(1.0-1.0/bdur) + gsl_matrix_get(sld,l,l-1)*(gsl_matrix_get(x,l,l) + tau*sxpjl))
				);
