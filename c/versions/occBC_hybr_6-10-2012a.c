...
double 	sig_psi	=3.671249;//1; 	// higher reduces the switching prob
...
double 	phi		= 0.455997;	// : log(1/UE elasticity -1) = phi*log(theta)= log(1/.38-1)/log(5)
double 	fm_shr	= 0.9;	// firm's surplus share
double 	effic	= 1.162658;
double 	sbar	= 0.026949;	// will endogenize this a la Cheremukhin (2011)

double rhoZ		= 0.9732;
double rhozz	= 0.99;
double sig_eps	= 2.41e-5;
double sig_zet	= 1.4548e-4;
...
int sol_ss(gsl_vector * ss, gsl_matrix * xss, struct sys_coef * sys){
	...
	for(iter=0;iter<itermax;iter++){
		...
		//either make ret_d local or do not parallelize
...
int sim_moments(struct aux_coef * simdat,gsl_vector * ss,gsl_matrix * xss,struct sys_sol * sol,struct sys_coef * sys){
	...
	if(urtss>0.01)
		...
		for(l=0;l<Noccs+1;l++){
			gsl_matrix_set(x,l,0,0.05/((double)Noccs+1.0));
			for(d=1;d<Noccs+1;d++)
				gsl_matrix_set(x,l,d,0.95/((double)(Noccs*Noccs+Noccs)) );
		}
	}
...
int theta(gsl_matrix * tld, const gsl_vector * ss, const struct sys_coef * sys, const struct sys_sol * sol, const gsl_vector * Zz){
	...
	for(l=0;l<Noccs+1;l++){
		...
		if(tld_s <= 0.0){
			t_zeros ++;
			if(l>0)
				gsl_matrix_set(tld,l,l-1,pow(pow(effic*fm_shr*(chi[l][d]*exp(Z)-bl)/kappa,phi)-1.0,1.0/phi) );
			else{
				for(d=0;d<Noccs;d++)
					gsl_matrix_set(tld,l,d,pow(pow(effic*fm_shr*(chi[l][d]*exp(Z)-bl)/kappa,phi)-1.0,1.0/phi) );
			}
		}
	}
...
//  Non- deriv based
	...
	if(alg1>0){
		opt1 = nlopt_create(NLOPT_LD_SLSQP,n);
		double x_tol1 = 1e-7;
		double f_tol1 = 1e-7;
		nlopt_set_xtol_rel(opt1, x_tol1);
		nlopt_set_ftol_rel(opt1, f_tol1);
		nlopt_set_maxeval(opt1, 50+pow(10,n));
		nlopt_set_stopval(opt1, 0.0);
		nlopt_set_lower_bounds(opt1,lb);
		nlopt_set_upper_bounds(opt1,ub);
		nlopt_set_min_objective(opt1, Vobj, (void*) ss);
	}
	...
	if(alg1>0 && opt_f>f_tol/10.0)
		status += nlopt_optimize(opt1, x0, &opt_f);
...
double cal_dist_wrap(double x, void* params){
	// DOES NOT WORK YET!!!!!!!!
	double *grad;
	grad = NULL;
	unsigned n = ((struct st_wr * )params)->n;
	return cal_dist(n, ((struct st_wr * )params)->x, grad, params);
}
