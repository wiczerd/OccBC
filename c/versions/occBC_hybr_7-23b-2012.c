...
double 	phi		= 0.421993;	// : log(1/UE elasticity -1) = phi*log(theta)= log(1/.38-1)/log(5)
	...
	for(iter=0;iter<itermax;iter++){
		...
		for(vfiter =0;vfiter<itermax/10;vfiter++){
			for(d=0;d<Noccs;d++){
				...
				gsl_vector_set(W_ld,l*Noccs+d,((1.0-sepld)*chi[l][d]*exp(Z+zd) + sepld*W_l0->data[l])/(1.0-(1.0-sepld)*beta));
			}
			// W_ld
			for(l=0;l<Noccs+1;l++){
				for(d=0;d<Noccs;d++){
					...
					if(l-1!=d){
						gsl_vector_set(W_ld,l*Noccs+d,
							((1.0-tau)*((1.0-sepld )*chi[l][d]*exp(Z+zd) + sepld*W_l0->data[0] ) + tau*W_ld->data[(d+1)*Noccs+d]  )
			...
			for(l=0;l<Noccs+1;l++){
				...
				for(d=0;d<Noccs;d++){
					...
					W_0 +=gld->data[l*Noccs+d]*gsl_matrix_get(pld,l,d)*(1.0-fm_shr)*ret;
				}
...
*/
		...
		for(l=0;l<Noccs+1;l++){
			...
			for(d=0;d<Noccs;d++){
				...
				double arr_d = chi[l][d]*exp(Z+zd) -nud - bl + post + beta*Wdif;
				//if(break_flag==1)
...
//	int Notz = 1 + Nagf + Nfac*(Nflag+1);
	...
	for (l=0;l<Noccs+1;l++){
		...
		double dWl0dZ =0.0;
		for(d=0;d<Noccs;d++)
			dWl0dZ += ss->data[ss_gld_i+l*Noccs+d]*chi[l][d]*pmatch(ss->data[ss_tld_i+l*Noccs+d])*(1.0-fm_shr);
