...
double 	sig_psi	=15.627577; // higher reduces the switching prob
...
*/
	...
	nu		= x[1];
	tau 	= x[2]; */
	double x0[]	= {effic,sig_psi,tau,sk_wg[0],sk_wg[1],sk_wg[2],sk_wg[3]};
	double lb[]	= {0.005,0.0,0.0001,x0[3]*(0.7),x0[4]*(0.7),x0[5]*(0.7)}; // what to do about sig_psi?  Skipping that right now
	double ub[]	= {1.0,1.0,0.05,x0[3]*(1.3),x0[4]*(1.3),x0[5]*(1.3)};
	for(i=0;i<6;i++){
		if (lb[i]>=ub[i]){
			double lbi = lb[i];
			lb[i] = ub[i];
