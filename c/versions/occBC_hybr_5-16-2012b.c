...
//double sk_wg[] = {-.05080318,.05512636,.03483991,.18627222};
	...
	*/
	...
	for(l=1;l<Noccs+1;l++){
		for(d=0;d<Noccs;d++){
			chi[l][d] = 0.0;
			for(i=0;i<Nskill;i++){
				chi[l][d] += sk_wg[i]*gsl_matrix_get(f_skills,d,i) + sk_wg[Nskill+i]*gsl_matrix_get(f_skills,l-1,i);
			}
...
/*	simdat.data_moments->data[0] = avg_sep;
	...
	double ub[]	= {1.0,1.0,0.05,x0[3]*(1.3),x0[4]*(1.3),x0[5]*(1.3),x0[3]*(1.3)};
	for(i=0;i<7;i++){
		if (lb[i]>=ub[i]){
			double lbi = lb[i];
			lb[i] = ub[i];
...
//	double objval = bndCalMinwrap(&cal_dist,lb,ub,7,x0,&st,1e-7,1e-7,0.0,opt_alg);
//	if (verbose >= 1)printf("Final Calibration Distance : %f\n",objval);
...
int sol_ss(gsl_vector * ss, gsl_matrix * xss, struct sys_coef * sys){
	...
	maxtss = pow(pow(1.0/0.99,phi)-1.0,1.0/phi);
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			gsl_vector_set(thetald,l*Noccs+d,pow(0.25/effic,1.0/(1.0-phi)));  //pow(kappa/(effic*(1.0-phi)*(chi[l][d] - gsl_vector_get(wld,l*Noccs+d))),-1.0/phi));
			gsl_matrix_set(pld,l,d,effic*thetald->data[l*Noccs+d]/pow(1+pow(thetald->data[l*Noccs+d],phi),1.0/phi) );
...
*/
		...
		for(l=0;l<Noccs+1;l++){
			...
			for(d=0;d<Noccs;d++){
				...
				tinner = tinner > 0 ? tinner : 0.0;
				//if(tinner > effic*pow(mintss,1.0-phi) )
				double theta =	pow(pow(kappa/(effic*fm_shr
						/tinner),phi)-1,1.0/phi) ;
	...
	*/
			...
			for(d=0;d<Noccs;d++){
				...
				double dtld = ss->data[ss_gld_i+l*Noccs+d]*(
								effic*(1.0 - pow(ss->data[ss_tld_i+l*Noccs+d],phi))/
...
int sys_co_diff(gsl_vector * ss, gsl_matrix * Dst, gsl_matrix * Dco, gsl_matrix* Dst_tp1, gsl_vector * xx){
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			surp = surp <0.0 ? 0.0: surp;
			gsl_matrix_set(Dco,tld_i + l*Noccs+d,tld_i + l*Noccs+d, 1.0);
			//dt/dWld
			double dtld =-beta*pow(pow(kappa/(fm_shr*effic*surp),phi)-1.0,1.0/phi-1.0 )*pow(kappa/(fm_shr*effic*surp),phi)/surp;
			...
			*/
			// dg/dt
			double dexret_d = sig_psi*exp(sig_psi*ret_d)*((1.0- pow(ss->data[ss_tld_i+l*Noccs+d],phi))/
					pow(1.0+pow(ss->data[ss_tld_i+l*Noccs+d],phi),1.0/phi+1.0)*arr_d
					-kappa*pow(1.0+ pow(ss->data[ss_tld_i+l*Noccs+d],phi),1.0/phi-1.0)*pow(ss->data[ss_tld_i+l*Noccs+d],phi-1.0))
					;
			if(ss->data[ss_tld_i+l*Noccs+d]>mintss + 5e-6 && arr_d>0.0)
				gsl_matrix_set(Dco,gld_i+l*Noccs+d,tld_i+l*Noccs+d,
					-1.0*(dexret_d*gdenom -dexret_d*exp(sig_psi*ret_d))/pow(gdenom,2)
				);
			else
			...
			// IS THIS THE PROBLEM???
				if(arr_dd>0.0)
					gsl_matrix_set(Dco,gld_i+l*Noccs+d,tld_i+l*Noccs+dd,
						-1.0*
						-sig_psi*exp(sig_psi*ret_dd)*(
						(1.0- pow(ss->data[ss_tld_i+l*Noccs+d],phi))/
						pow(1.0+pow(ss->data[ss_tld_i+l*Noccs+d],phi),1.0/phi+1.0)*arr_d
						-kappa*pow(1.0+ pow(ss->data[ss_tld_i+l*Noccs+d],phi),1.0/phi-1.0)*pow(ss->data[ss_tld_i+l*Noccs+d],phi-1.0)
						)*exp(sig_psi*ret_dd) /pow(gdenom,2)
					);
				else
	...
	for (l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			gsl_matrix_set(Dco,tld_i + l*Noccs+d,0,   // dtheta/dZ
				chi[l][d]*pow(pow(kappa/(fm_shr*effic*surp),phi)-1.0,1.0/phi-1.0 )*pow(kappa/(fm_shr*effic*surp),phi)/surp
				);
			...
			double dnum 	= sig_psi*pld*chi[l][d]*exp(sig_psi*ret_d);
			double ddenom	= sig_psi*pld*chi[l][d]*exp(sig_psi*ret_d);
			...
			if(dd!=d){
				double nudd = l==dd+1 ? 0: nu;
				double p_dd		= ss->data[ss_tld_i+l*Noccs+d]/pow(1.0+pow(ss->data[ss_tld_i+l*Noccs+d],phi),1.0/phi);
				double postdd	= - kappa*ss->data[ss_tld_i+l*Noccs+dd]/p_dd;
				double contval	= (ss->data[ss_Wld_i+l*Noccs+dd]-ss->data[ss_Wl0_i+l]);
				//contval = contval<0.0 ? 0.0 : contval;
				...
				double ddenom	= sig_psi*p_dd*chi[l][dd]*exp(sig_psi*ret_dd);
...
int sim_moments(struct aux_coef* simdat,gsl_vector * ss,gsl_matrix * xss,struct sys_sol * sol,struct sys_coef * sys){
	...
	gsl_matrix * XX = gsl_matrix_calloc(Noccs*Noccs,2*Nskill+1);
	gsl_vector_view X0 = gsl_matrix_column(XX,2*Nskill);
	...
	gsl_vector * coefs	= gsl_vector_calloc(2*Nskill + 1);
	gsl_vector * coefs_di	= gsl_vector_calloc(2*Nskill + 1);
	...
	for(di = 0;di<Ndraw;di++){
		...
		*/
		...
		d_chng = d_chng!=d_chng ? s_chng*((double)Ndraw/((double)di+1.0) ) : d_chng;
...
//	Wage growth
		...
		d_wg = d_wg != d_wg ? s_wg*((double)Ndraw )/((double)di +1.0) : d_wg;
	...
	*/
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			double post = -kappa/effic*pow(gsl_matrix_get(sol->tld,l,d),phi);
			double gld_ld 	=exp(sig_psi*gsl_matrix_get(pld,l,d)*
					(chi[l][d]*exp(Z + zd) - bl - nud +post + beta*(Es->data[Wld_i + l*Noccs+d] -Es->data[Wl0_i + l])) );
			gld_ld 	= gld_ld < 0.0 ? 0.0 : gld_ld;
			gld_ld 	= gld_ld != gld_ld ? 0.0:gld_ld;
			gdenom += gld_ld;
		}
		for(d=0;d<Noccs;d++){
			double nud = l == d+1? 0:nu;
			...
			double post = -kappa/effic*pow(gsl_matrix_get(sol->tld,l,d),phi);
			double gld_ld =exp(sig_psi*gsl_matrix_get(pld,l,d)*
					(chi[l][d]*exp(Z + zd) - bl - nud + post + beta*(Es->data[Wld_i + l*Noccs+d] -Es->data[Wl0_i + l])) )/gdenom;
...
int theta(gsl_matrix * tld, const gsl_vector * ss, const struct sys_coef * sys, const struct sys_sol * sol, const gsl_vector * Zz){
	...
	double mul = pow(effic*fm_shr/kappa,1.0/phi);
	for(l=0;l<Noccs+1;l++){
		double bl = l>0 ? b[1]:b[0];
		for(d=0;d<Noccs;d++){
			double nud = l == d+1? 0:nu;
			double zd = Zz->data[d+1+Ntfpl+Nfac*(Nflag)];
			double tld_i =mul*pow(chi[l][d]*exp(Z+zd) - bl - nud + beta*(Es->data[Wld_i+l*Noccs+d] - Es->data[Wl0_i+l]),1.0/phi);
			tld_i = tld_i>=0 ? tld_i:1e-5;
			tld_i = tld_i<= pow(effic,1.0/(phi-1.0)) ? tld_i : pow(effic,1.0/(phi-1.0))-0.001;
			tld_i = gsl_finite(tld_i) ? tld_i : 0.0;
...
int xprime(gsl_matrix * xp, gsl_vector * ss, const struct sys_coef * sys, const struct sys_sol * sol, const gsl_matrix * x, const gsl_vector * Zz){
	...
	for(l=0;l<Noccs+1;l++){
		findrt[l]=0.0;
		for(d=0;d<Noccs;d++)
			findrt[l] += effic*pow(gsl_matrix_get(sol->tld,l,d),1.0-phi)*gsl_matrix_get(sol->gld,l,d);
	}
		...
		for(d=0;d<Noccs;d++){
			if(l!=d+1)
				gsl_matrix_set(xp,l,d+1,
					(1.0-tau)*(1.0-sbar)*gsl_matrix_get(x,l,d+1) + effic*pow(gsl_matrix_get(sol->tld,l,d),1.0-phi)*ald[l][d]
					);
			else if(l==d+1){
				double newexp = 0.0;
				for(j=0;j<Noccs+1;j++)
				...
				gsl_matrix_set(xp,l,d+1,
					(1.0-sbar)*(gsl_matrix_get(x,l,d+1)+newexp )+ effic*pow(gsl_matrix_get(sol->tld,l,d),1.0-phi)*ald[l][d]
					);
			}
		}
	}
...
//	b		= x[6];
	...
	for(l=1;l<Noccs+1;l++){
		for(d=0;d<Noccs;d++){
			chi[l][d] = 0.0;
			for(i=0;i<Nskill;i++){
				chi[l][d] += x[3+i]*gsl_matrix_get(f_skills,d,i) - x[3+i+Nskill]*gsl_matrix_get(f_skills,l-1,i);
			}
			chi[l][d] += x[3+2*Nskill];
		}
	}
	double chi_lb = 0.01;
	double chi_ub = 0.99;
