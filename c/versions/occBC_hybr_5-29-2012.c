...
double Zfcoef[] = {-0.0028,-0.0089,0.0355,0.0083};
...
*/
	...
	sig_psi = x[1];
	tau 	= x[2]; */
	double x0_0[]	= {effic,sig_psi,tau,sk_wg[0],sk_wg[1],sk_wg[2],sk_wg[3],sk_wg[4]};
	...
	double objval = bndCalMinwrap(&cal_dist,lb_1,ub_1,3,x0_1,&st,1e-7,1e-7,0.0,opt_alg);
...
//	int wld_i, tld_i, x_i,gld_i;
	...
	for(f=0;f<Nfac;f++)
			gsl_matrix_set(N1,0,1+Ntfpl+Nfac*Nflag,Zfcoef[f+Nfac]);
