	...
	for(iter=0;iter<itermax;iter++){
		...
		for(l=0;l<Noccs+1;l++){
			...
			for(d=0;d<Noccs;d++){
				...
				theta = theta > pow(1.0/effic,1.0/(1.0-phi)) ? pow(0.999/effic,1.0/(1.0-phi)): theta;
...
int sys_st_diff(gsl_vector * ss, gsl_matrix * Dst, gsl_matrix * Dco, gsl_matrix* Dst_tp1, gsl_vector * xx){
	...
	for(l=0;l<Noccs+1;l++){
			...
			for(d=0;d<Noccs;d++){
				...
				gdenom +=exp(sig_psi*effic*pow(ss->data[ss_tld_i+l*Noccs+d],1.0-phi)*(chi[l][d]-b+nud+ post + beta*contval));
			}
	/*		// xl0
...
// derivatives for exog
	...
	for (l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			double tinside = (chi[l][d]-nud- b + post +beta*(ss->data[ss_Wld_i+l*Noccs+d]-ss->data[ss_Wl0_i+l]))*effic*(1.0-phi)/kappa;
