...
int printlev 	= 3;
int use_anal	= 1;
int opt_alg		= 0; // 10x => Nelder-Meade, 5x => Subplex, 3x => DFBOLS, o.w => BOBYQA
int polish_alg	= 0;
...
double 	sig_psi	= 0.479879; 	// higher reduces the switching prob

double *	b; // unemployment benefit

//At phi=0.476083,sig_psi=0.479879,tau=0.016685,scale_s=0.029920,shape_s=0.037902,effic=0.886338

double ** chi;
double 	tau 	= 0.016685;
double 	kappa	= 0.26;//.1306; corresponds to HM number
double 	phi		= 0.476083;	// : log(1/UE elasticity -1) = phi*log(theta)= log(1/.38-1)/log(5)
double 	fm_shr	= 0.66;	// firm's surplus share
double 	effic	= 0.886338;
double 	sbar	= 0.02;	// will endogenize this a la Cheremukhin (2011)

double 	scale_s	= 0.029920;
double 	shape_s	= 0.037902;
...
int main(int argc,char *argv[]){
	...
	double chi_coef0[] = {0.01,0.005,0.005};
...
*/
		...
		for(l=0;l<Noccs+1;l++){
			...
			for(dd=0;dd<Noccs;dd++)
				sexpret_dd = gsl_vector_get(ret_d,dd)>0.0 ?
			...
			for(d=0;d<Noccs;d++){
				...
				if(ret_d->data[d]<=0)
					gg =0;
				double distg = fabs(gg - gld->data[l*Noccs+d])/(1.0+gld->data[l*Noccs+d]);
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			wld_ld = (1.0 - fm_shr)*(W_ld->data[l*Noccs+d] - W_l0->data[l]);
			...
			for(l=1;l<Noccs+1;l++){
				for(d=0;d<Noccs;d++){
					if(l-1!=d){
						...
						gsl_vector_set(yloss,Xrow,
								log(gsl_matrix_get(sol->ss_wld,l,d)/gsl_matrix_get(sol->ss_wld,d+1,d)));
