...
double 	effic	= 0.780289;//0.747769;
		...
		for(l=0;l<Noccs+1;l++){
			...
			for(d=0;d<Noccs;d++){
				...
				W_0 +=gld->data[l*Noccs+d]*gsl_matrix_get(pld,l,d)*(chi[l][d] - nud
						- kappa/effic*pow(gsl_matrix_get(pld,l,d),1.0+phi)
...
int sys_st_diff(gsl_vector * ss, gsl_matrix * Dst, gsl_matrix * Dco, gsl_matrix* Dst_tp1, gsl_vector * xx){
	...
	for(l=0;l<Noccs+1;l++){
			...
			for(d=0;d<Noccs;d++){
				...
				gdenom +=exp(sig_psi*effic*pow(ss->data[ss_tld_i+l*Noccs+d],1.0-phi)*(chi[l][d]-b+nud+beta*contval));
			}
	/*		// xl0
	...
	*/
			...
			for(d=0;d<Noccs;d++){
				...
				// ss_ret = -nu\indic - kappa/theta^ld/p^ld + mu^ld - mu^l0
				double ss_ret = d+1==l ? 0.0 : -nu;
				ss_ret += chi[l][d]*exp(Z+zd) -b - kappa/effic*pow(ss->data[ss_tld_i+l*Noccs+d],1.0+phi);
...
int sys_co_diff(gsl_vector * ss, gsl_matrix * Dst, gsl_matrix * Dco, gsl_matrix* Dst_tp1, gsl_vector * xx){
	...
	for(l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			zd = xx->data[d+1];
			double nud = l==d+1 ? 0: nu;
			double post=  - kappa/effic*pow(ss->data[ss_tld_i+l*Noccs+d],1.0+phi);
			// tld
			double tinside = (chi[l][d]-nud- b + post +beta*(ss->data[ss_Wld_i+l*Noccs+d]-ss->data[ss_Wl0_i+l]))
					*effic*(1.0-phi)/kappa;
			...
			*/
			...
			if(dd!=d){
				...
				contval <0.0 ? 0.0 : contval;
				double postdd	=  - kappa/effic*pow(ss->data[ss_tld_i+l*Noccs+dd],1.0+phi);
...
// derivatives for exog
	...
	for (l=0;l<Noccs+1;l++){
		...
		for(d=0;d<Noccs;d++){
			...
			double arr_d 	= (chi[l][d] -b+nud+beta*contval);
			...
			if(dd!=d){
				...
				double arr_dd 	= (chi[l][dd]-b+nudd+beta*contval);
