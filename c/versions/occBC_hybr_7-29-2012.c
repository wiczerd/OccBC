...
double 	phi	= 0.436605;	// : log(1/UE elasticity -1) = phi*log(theta)= log(1/.38-1)/log(5)
double 	sig_psi	= 0.023969; 	// higher reduces the switching prob
double 	tau 	= 0.050260;
double 	scale_s	= 0.032846;
double 	shape_s	= 0.091122;
double 	effic	= 0.53266;
...
int main(int argc,char *argv[]){
	...
	if(homosk_zeta==1){
		...
		gsl_matrix_scale(var_zeta,1.2495); // adjust so that has the same unconditional variance as with the true persistence
	}
	else
		readmat("var_zeta.csv",var_zeta);
	...
	*/
	...
	double lb_0[]	= {0.35	,0.001		,0.005	,0.015		,0.005		,0.25	,-5.0			,-5.0			,-5.0	}; // what to do about sig_psi?  Skipping that right now
	double ub_0[]	= {0.8	,3.5		,0.06	,0.05		,0.20		,1.25	,.01			,.01			,.01	};
