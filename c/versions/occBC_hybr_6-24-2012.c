...
/*	double chi_lb = 0.01;
	...
	for(l=1;l<Noccs+1;l++){
		for(d=0;d<Noccs;d++){
			...
			if(chi[l][d]>=chi_ub)
				child = chi_ub;
			chi[l][d] = (1.0 - child); // there is some selection, so the actual productivity is lower.
...
#ifdef _MKL_USE
	int * ipiv = (int*)malloc(sizeof(int)*c);
	status =LAPACKE_dgetrf(LAPACK_ROW_MAJOR, r,c, F0dat, c,ipiv );
#endif

	status += sol_AXM(gsl_matrix * A, gsl_matrix * M, gsl_matrix *X, int* tri);
