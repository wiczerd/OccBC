...
double 	sig_psi	= 2.865434;//3.671249; 	// higher reduces the switching prob
...
double 	phi		= .30;//0.455997;	// : log(1/UE elasticity -1) = phi*log(theta)= log(1/.38-1)/log(5)
double 	fm_shr	= 0.75;	// firm's surplus share
double 	effic	= 1.174104;//1.162658;
double 	sbar	= 0.026949;	// will endogenize this a la Cheremukhin (2011)

double 	scale_s	= 0.03;
double 	shape_s	= 0.0010;
...
int main(){
	...
	double chi_coef0[] = {0.01,0.01,0.00};
...
int sol_ss(gsl_vector * ss, gsl_matrix * xss, struct sys_coef * sys){
	...
	for(iter=0;iter<itermax;iter++){
		for(d=0;d<Noccs;d++){
			...
			//else
			//	gsl_vector_set(W_ld,l*Noccs+d,(1.0-sepld)*(chi[l][d] +W_ld->data[l*Noccs+d]) + sepld*W_l0->data[l]);
		}
		// W_ld
		for(l=0;l<Noccs+1;l++){
			for(d=0;d<Noccs;d++){
			...
			//	else
			//		gsl_vector_set(W_ld,l*Noccs+d,
			//			(1.0-tau)*((1.0-sepld )*(chi[l][d] + beta*W_ld->data[l*Noccs+d] ) + sepld*W_l0->data[0] )
			//			+ tau*W_ld->data[(d+1)*Noccs+d]);
		...
		for(l=0;l<Noccs+1;l++){
			...
			for(d=0;d<Noccs;d++){
				...
				// double theta =pow(effic*fm_shr*surp/kappa,phi)-1.0 ;
				//if(effic>1.0)
				//	theta = theta>1.0/(pow(effic,phi)-1.0)? 1.0/(pow(effic,phi)-1.0) : theta;
				//if(surp > 0.0 && gsl_finite(theta) && theta>0.0 ){
		...
		for(l=0;l<Noccs+1;l++){
			...
			//}
			W_l0->data[l] = W_0;
			if(fabs(W_0 - lW_l0->data[l])/lW_l0->data[l] > maxdist)
...
int sim_moments(struct st_wr * st, gsl_vector * ss,gsl_matrix * xss){
	...
	if(verbose>=2){
		...
		printf("\n");
		double dispobj =0.0;
		for(l=0;l<(simdat->data_moments)->size1;l++)
...
double bndCalMinwrap(double (*Vobj)(unsigned n, const double *x, double *grad, void* params),
	...
	int status,resi,starts,i,locstatus,dfbols_flag;
	...
	if(alg0>=10){//random restarts with MPI -- here's where MPI branching begins.
		...
		for(resi=0;resi<starts;resi++){
			...
			if((d_xres>d_inner && d_xmin<d_outer) || resi%5==0){
				...
				if(dfbols_flag==1){
						bobyqa_h(&n,&npt,x0,lb,ub,&rhobeg,&rhoend,&iprint,&maxfun,w,&mv);
				}
				else
