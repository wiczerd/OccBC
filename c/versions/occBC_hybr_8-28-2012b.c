...
int main(int argc,char *argv[]){
	...
	*/
	...
	double lb_0[]	= {0.25	,0.0005		,0.005	,0.02		,0.005		,0.5	,-2.0			,-2.0			,-2.0	};
	double ub_0[]	= {0.8	,2.5		,0.15	,0.08		,0.20		,1.35	,.00			,.00			,.00	};
...
*/	//readvec("outzz.csv",Zz_TGR);
	...
	for(di=0;di<Ndraw;di++){
		...
		for(l=0;l<Nx;l++){
			...
			double slope_sim0	= (zd2009-gsl_matrix_get(Zhist,0,l))/12;
			double slope_dat0 	= (gsl_vector_get(Zz_2009,l)-gsl_vector_get(Zz_2008,l))/12;
			double zd2010 		= gsl_matrix_get(Zhist,23,l);
			double slope_sim1	= (zd2009-gsl_matrix_get(Zhist,0,l))/12;
			double offset2009	= gsl_vector_get(Zz_2009,l) - gsl_matrix_get(Zhist,11,l);
			...
			/*for(t=12;t<Tmo;t++){
				double Zhist_d = gsl_matrix_get(Zhist,t,l);
				gsl_matrix_set(Zhist,t,l, Zhist_d - zd2009 + gsl_matrix_get(Zhist,11,l));
				Zave->data[t*Nx + l] += Zhist->data[t*Nx +l]/(double)Ndraw;
				gsl_matrix_set(Zpool,di*Tmo + t,l,Zhist->data[t*Nx +l]);
			}*/
		}
...
double bndCalMinwrap(double (*Vobj)(unsigned n, const double *x, double *grad, void* params),
	...
	nlopt_set_maxeval(opt0, 250*pow(n,2));
...
double cal_dist(unsigned n, const double *x, double *grad, void* params){
	...
	if(exploding>=5 && alg!=35 && alg!=19)
		nlopt_force_stop(st->opt0);
	if(Nsolerr>2 || (dist>1000 && Nsolerr>1 )  && alg!=35 && alg!=19 )
		nlopt_force_stop(st->opt0);
